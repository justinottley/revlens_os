...
def _init():
    ...
    for moduleName in [
        ...
        libMod.init_module()
        rlp.core.util.loft_to_module(__name__, libMod)
    ...
    if presentation_win:
        ...
        for screen_info in _UI['menu.window.screens']:
            if screen_info['act'].isChecked():
                ...
                presentation_win.setViewScreen(screen_info['screen'])
...
def on_toggle_tab_headers():
    ...
    mv = revlens.GUI.mainView()

    mv.setTabHeadersVisible(not mv.tabHeadersVisible())

    # Terrible hack to redraw the screen, fixes misdrawn viewer and timeline
    mv.resizeView(mv.width(), mv.height() + 1)
    mv.resizeView(mv.width(), mv.height() - 1)
    mv.update()
...
def on_toggle_splitter_autohide():
    ...
    revlens.GUI.mainView().setSplittersVisible(not revlens.GUI.mainView().splittersVisible())
...
def show_plugin_ui(md):
    ...
    if pane_module:
        ...
        pane = revlens.GUI.mainView().createFloatingPane(400, 300, False)
...
def add_onthego_track():
    revlens.CNTRL.session().addTrackByType('OnTheGo')
...
def on_audio_scrubbing_toggled(*args):
    revlens.CNTRL.getAudioManager().toggleAudioScrubbing()

def on_preroll_toggled(*args):
    revlens.cmds.toggle_preroll(updateMenu=False)


def clear_global_ui():

    print('Clearing all UI objects')
    global _UI
    _UI = {}
...
def on_layout_load_triggered(layout_name):
    ...
    doInitView = False
    app_globals = RlpUtil.APPGLOBALS.globals()
    if 'gui.arrangement' in app_globals:
        doInitView = True

    revlens.GUI.loadArrangement(arrangement_data, doInitView)
...
def init_menu_default_layout(init_session_media_track=True, init_session_annotation_track=True):

    revlens._init_session(
        media=init_session_media_track,
        annotation=init_session_annotation_track
    )
    ...
    act_create_track_onthego = menu_create_track.addAction('On The Go')
    act_create_track_onthego.triggered.connect(functools.partial(on_triggered, add_onthego_track))
    ...
    act_preroll = menu_control.addAction('Preroll')
    act_preroll.setCheckable(True)
    act_preroll.triggered.connect(on_preroll_toggled)
    ...
    _UI['menu.control.preroll'] = act_preroll
    ...
    menu_audio = revlens.GUI.addMenu('Audio')
    act_audio_scrubbing = menu_audio.addAction('Scrubbing')
    act_audio_scrubbing.setCheckable(True)
    act_audio_scrubbing.triggered.connect(on_audio_scrubbing_toggled)
    ...
    screen_list = QtGui.QApplication.instance().screens()
    ...
    for layout_entry in sorted(layout_list):
...
def init_menu_global():
    ...
    revlens_icon = RlpGui.GUI_IconButton(':misc/revlens_logo_grey_1.png', menu_bar, 20, 0)
    ...
    launcher_item = revlens_menu.addItem('Start Launcher', {}, False)
    launcher_item.triggered.connect(start_launcher)
...
def init_menu_project_list():
    ...
    projectName = RlpUtil.APPGLOBALS.value('project.name') or ''
    if _UI.get('menu.projlist'):

        proj_list = _UI['menu.projlist']
        if projectName:
            proj_list.setText(projectName)
            proj_list.showItem()
    ...
    proj_list.setSvgIconPath(':feather/package.svg', 20, True)
    proj_list.setText(projectName)
    ...
    def _menu_item_selected(md):
        ...
        act_name = md['data']['name']
        if act_name == 'new_project':
            revlens.GUI.mainView().emitSelectionChanged(
                {
                    'source': 'project_dropdown',
                    'data': 'select_project'
        ...
        elif act_name == 'settings':

            # TODO FIXME: illegal import
            from revlens_prod.dialogs import ProdDataSettingsDialog
            ProdDataSettingsDialog.create()
    ...
    act_settings = proj_list_menu.addItem('Settings..', {'name': 'settings'}, False)
...
def show_load_dialog(sel_data):
    ...
    floating_pane = revlens.GUI.mainView().createFloatingPane(400, 280, False)
