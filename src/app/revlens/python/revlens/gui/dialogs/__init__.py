...
class ExportDialog(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent):
        ...
        self.output_format.setSvgIconPath(':feather/chevron-down.svg', 20, True)
    ...
    def create(cls, width=400):
        ...
        pane = revlens.GUI.mainView().createFloatingPane(width, 580, False)
        panel = cls(pane)
...
class AboutDialog(RlpGui.GUI_ItemBase):
    ...
    def create(cls, width=400):
        ...
        pane = revlens.GUI.mainView().createFloatingPane(width, 120, False)
        panel = cls(pane)
...
class SetupDialog(RlpGui.GUI_ItemBase):
    ...
    def create_desktop_shortcut(self, md=None):
        ...
        appRootPath = RlpUtil.APPGLOBALS.value('app.rootpath')
        print('Got App Root Path: {}'.format(appRootPath))
        ...
        try:
            ...
            if not os.path.isfile(top_launcher_path):
                ...
                working_dir = os.path.dirname(os.path.dirname(appRootPath))
    ...
    @classmethod
    def create(cls, width=400):
        pane = revlens.GUI.mainView().createFloatingPane(width, 220, False)
        panel = cls(pane)
...
class LoadDialog(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, title, media_info, floating_pane):
        ...
        self.loader.setLoadInfo(media_info)
    ...
    @classmethod
    def create(cls, file_path, media_name=None):
