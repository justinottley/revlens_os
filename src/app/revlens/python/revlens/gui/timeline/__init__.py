...
def solo_track(trackUuid):
    ...
    for tlViewName in tlViewNames:
        ...
        for trackIdx in tlMgr.trackIndexes():
            ...
            toggled = tlTrack.uuid().toString() != trackUuid
            visButton.setToggled(toggled)
            visButton.update()
            
            tlStrack = revlens.CNTRL.session().getTrackByUuid(tlTrack.uuid().toString())
            tlStrack.setEnabled(False, False)

    strack = revlens.CNTRL.session().getTrackByUuid(trackUuid)
    strack.setEnabled(True, True)
...
def request_remove_item_at_current_frame(track):
    ...
    def _on_accept(md=None):
        ...
        track.removeNodeByFrame(curr_frame, True)
