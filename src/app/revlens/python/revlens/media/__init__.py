...
def sanitize_session(session_input, remove_paths=False):
    ...
    for track_info in list(session_input['track_data'].values()):
        ...
        for media_info in list(track_media.values()):
            ...
            if 'graph' in media_info:
                del media_info['graph']

            del_keys = []
            for key, value in list(media_info['properties'].items()):
                if key.startswith('media.thumbnail') or not key.startswith('media.'):
                    del_keys.append(key)

            for del_key in del_keys:
                del media_info['properties'][del_key]
...
def append_to_composite(media_in, node_uuid, composite_mode='Tile'):
    ...
    media_result_val = mmgr.getMediaFactory().identifyMedia(media_in)
    media_result = media_result_val
    ...
    if media_result:
        node = mmgr.getMediaFactory().createMedia(media_result_val, True)
        if node:
            
            rlogger.info('{}: Appending to {}'.format(node, composite_node))
            composite_node.addInput(node)
            # composite_node.insertProperty("display.composite_mode", composite_mode)
            ...
            revlens.CNTRL.emitNoticeEvent(
                'view.request_composite',
                {'action': 'set_mode', 'composite_mode': 'Tile'}
            )
