...
PLUGIN_COMPOSITE_MODE = 10
PLUGIN_VIEW_TOOLBAR = 11
PLUGIN_GRID_ROW_WIDGET = 12
...
def register(plugin_type, plugin_item):
    ...
    if (plugin_type == PLUGIN_EVENT):
        ...
        if revlens.GUI:
            result = revlens.GUI.registerToolGUI(plugin_item)
    ...
    elif (plugin_type == PLUGIN_COMPOSITE_MODE):
        plugin_manager.registerCompositePlugin(plugin_item)
    ...
    elif (plugin_type == PLUGIN_VIEW_TOOLBAR):
        import revlens.gui
        revlens.gui.GUI_VIEW_TOOLBARMANAGER_OBJ.registerViewerToolbar(plugin_item)

    elif (plugin_type == PLUGIN_GRID_ROW_WIDGET):
        RlpGui.GUI_GRID_ROWFACTORY_OBJ.registerRowWidgetPlugin(
            plugin_item['name'],
            plugin_item['plugin']
        )
