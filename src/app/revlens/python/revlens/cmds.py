...
def clear_in_out_frames():
    ...
    inFrame = 1
    outFrame = session.frameCount()

    stickyBookends = False
    if revlens.CNTRL.session().hasProperty('bookends.sticky'):
        stickyBookends = session.getProperty('bookends.sticky')

    if stickyBookends:
        soloTrack = session.getSoloedTrackByType(revlens.TRACK_TYPE_MEDIA)
        if soloTrack:
            outFrame = soloTrack.getMaxFrameNum()



    # controller update controller by signal-slot connection
    #
    revlens.CNTRL.setInFrame(inFrame, False, True)
    revlens.CNTRL.setOutFrame(outFrame, False, True)
...
def toggle_mute():
    md = {'button': -1, 'toggled': not revlens.CNTRL.getAudioManager().isMuted()}
    revlens.CNTRL.emitNoticeEvent('audio.request_toggle_mute', md)


def toggle_preroll(updateMenu=True):

    import revlens
    vmgr = revlens.CNTRL.getVideoManager()
    currPrerollCount = vmgr.prerollFrameCount()

    prerollOn = False
    if currPrerollCount == 0:
        prerollOn = True
        vmgr.setPrerollFrameCount(24)
        display_message('Preroll ON')
        

    else:
        vmgr.setPrerollFrameCount(0)
        display_message('Preroll OFF')

    if updateMenu:
        import revlens.gui.menu
        act_preroll = revlens.gui.menu._UI['menu.control.preroll']
        act_preroll.setChecked(prerollOn)
