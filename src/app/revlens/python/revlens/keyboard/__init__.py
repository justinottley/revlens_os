...
class KeyEntry(object):
    ...
    def __init__(self, name, text, modifiers, key, platform=None, **kwargs):
    ...
    def update(self, text, modifiers, key, platform=None, **kwargs):
    ...
    @classmethod
    def get_key_entry(cls, name):
        return cls.KEY_MAP.get(name)


    @classmethod
    def register_key_entry(cls, name, text, modifiers, key, platform=None, **kwargs):
        ...
        if kentry:
            kentry.update(text, modifiers, key, platform, **kwargs)

        else:

            ke = KeyEntry(name, text, modifiers, key, platform, **kwargs)
            cls.KEY_MAP[name] = ke
...
def handle_key_press_event(key_event, *args):
    ...
    if (revlens.GUI.mainView().hasFocusItem()):
        key_event['widgetItem'] = RlpGui.GUI.mainView().focusItem()
