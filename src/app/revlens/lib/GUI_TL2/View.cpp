    ...
    _uuidStr(QUuid::createUuid().toString())
{
    ...
    */
    ...
    GUI_TL2_Controller::instance()->registerTimelineView(this);
...
GUI_TL2_View::~GUI_TL2_View()
{
    ...
    GUI_TL2_Controller::instance()->deregisterTimelineView(this);
...
GUI_TL2_View::initConnections()
{
    ...
    connect(_playhead, &GUI_TL2_Playhead::scrollWheelDelta, this, &GUI_TL2_View::onScrollWheelDelta);
...
void
GUI_TL2_View::onScrollWheelDelta(int delta)
{
    float posPercent = (float)(delta * 0.25) / height();

    _yscrollPos += posPercent;
    if (_yscrollPos < 0)
    {
        _yscrollPos = 0;
    } else if (_yscrollPos > 1)
    {
        _yscrollPos = 1;
    }

    _vbar->setScrollPos(_yscrollPos);
    _trackManager->refresh();
}
...
GUI_TL2_View::onSessionChanged(DS_SessionPtr session)
{
    ...
    connect(
        ...
        this,
        &GUI_TL2_View::onTrackDataChanged
    );
...
void
GUI_TL2_View::onMediaDataReady(QString dataType, QVariantMap nodeInfo)
{
    if (dataType == "media_list")
    {
        refresh();
    }
}
...
void
GUI_TL2_View::onTrackDataChanged(QString evtName, QString trackUuidStr, QVariantMap info)
{
    if (evtName == "set_enabled")
    {
        // RLP_LOG_DEBUG("GOT SET ENABLED")
        refresh();
    }
}
...
void
GUI_TL2_View::onNoticeEvent(QString evtName, QVariantMap evtInfo)
{
    if (evtName == "playlist.changed")
    {
        _trackManager->refresh(0, /* updateEnabledTracks */ true);

    } else if (evtName == "playlist.selection_updated")
    {
        QString selMode = evtInfo.value("mode").toString();
        QVariantList selMedia = evtInfo.value("sel_info").toList();

        _trackManager->refreshSelection(selMode, selMedia);
