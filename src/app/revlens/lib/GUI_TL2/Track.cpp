...
void
GUI_TL2_Track::setTrackEnabled(bool enabled)
{
...
GUI_TL2_Track::onTrackDataChanged(QString evtName, QString trackUuid, QVariantMap changeInfo)
{
    ...
    else if (evtName == "media_updated")
    {
        QVariantList mediaChangeList = changeInfo.value("media").toList();
        updateMediaIndex(mediaChangeList);

        // Hack to actually get the timeline to repaint....
        //
        QTimer::singleShot(10, this, [this]() {

            int tXPos = this->view()->timelineXPosStart();
            this->view()->setTimelineXPosStart(tXPos - 1);
            this->view()->setTimelineXPosStart(tXPos  + 1);
...
void
GUI_TL2_Track::updateMediaIndex(QVariantList mediaChangeList)
{
    QVariantMap startNodeInfo = mediaChangeList.at(0).toMap();
    qlonglong startFrame = startNodeInfo.value("frame").toLongLong();

    QList<qlonglong> frames = _itemFrameMap.keys();

    for (auto frame : frames)
    {
        if (frame >= startFrame)
        {
            // RLP_LOG_DEBUG("Clearing node at" << frame)
            _itemFrameMap.remove(frame);
        }
    }

    for (int i = 0; i != mediaChangeList.size(); ++i)
    {
        QVariantMap nodeInfo = mediaChangeList.at(i).toMap();
        qlonglong nodeFrame = nodeInfo.value("frame").toLongLong();
        qlonglong frameCount = nodeInfo.value("frame_count").toLongLong();

        QUuid nodeUuid = QUuid(nodeInfo.value("node_uuid").toString());


        GUI_TL2_Item* item = _itemMap.value(nodeUuid);
        if (item == nullptr)
        {
            RLP_LOG_WARN("NO ITEM AT UUID" << nodeUuid)
            continue;
        }
        
        _itemFrameMap[nodeFrame] = nodeUuid;
        // RLP_LOG_DEBUG("setting frame range for item:" << nodeFrame << frameCount);

        item->setStartFrame(nodeFrame);
        item->setEndFrame(nodeFrame + frameCount);
    }
}
...
GUI_TL2_Track::addItem(GUI_TL2_Item* item)
{
    ...
    if (_itemMap.contains(item->uuid()))
    {
        RLP_LOG_WARN("Skipping update, item already present")
        return;
    }


    // Q_ASSERT(!(_itemFrameMap.contains(item->startFrame())));
    if (_itemFrameMap.contains(item->startFrame()))
    {
        RLP_LOG_WARN("Skipping update, item already present")
        return;
    }
