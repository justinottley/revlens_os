...
void
GUI_TL2_TrackManager::refresh(qreal iheight, bool updateEnabledTracks)
{
    ...
    {
        RLP_LOG_WARN("skipping refresh - GUI tracks: " << numTracks << " session tracks: " << numSTracks);
        return;
    }
    ...
    {
        DS_TrackPtr strack = trackList.at(x);
        GUI_TL2_Track* tlTrack = getTrackByUuid(strack->uuid().toString());

        if (updateEnabledTracks) // potentially expensive due to force updating all items
        {
            tlTrack->setTrackEnabled(strack->isEnabled());
        }
...
void
GUI_TL2_TrackManager::refreshSelection(QString selMode, QVariantList selMedia)
{
    // RLP_LOG_DEBUG(selMode);
    ...
    for (int i = 0; i != selMedia.size(); ++i)
    {
        QVariantMap mediaInfo = selMedia.at(i).toMap();

        if (mediaInfo.contains("track_uuid"))
        {
            QString trackUuidStr = mediaInfo.value("track_uuid").toString();
            if (_trackUuidMap.contains(trackUuidStr))
            {
                GUI_TL2_Track* tlTrack = _trackUuidMap.value(trackUuidStr);

                QString mediaItemUuid = mediaInfo.value("uuid").toString();
                GUI_TL2_Item* mediaItem = tlTrack->getItemByUuid(mediaItemUuid);
                if (mediaItem != nullptr)
                {
                    mediaItem->setItemEnabled(false);
                }
            }
        }
    }
