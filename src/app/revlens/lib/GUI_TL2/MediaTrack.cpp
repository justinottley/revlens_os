...
GUI_TL2_MediaTrack::initTrackConnections()
{
    ...
    connect(
        _strack.get(),
        &DS_Track::trackCleared,
        this,
        &GUI_TL2_MediaTrack::clearItems
    );

    connect(
        _strack.get(),
        &DS_Track::mediaRemoved,
        this,
        &GUI_TL2_MediaTrack::onMediaRemoved
    );

    connect(
        _strack.get(),
        &DS_Track::mediaEnabledChanged,
        this,
        &GUI_TL2_MediaTrack::onMediaEnabledChanged
    );

    connect(
        _strack.get(),
        &DS_Track::mediaSelectionChanged,
        this,
        &GUI_TL2_MediaTrack::onMediaSelectionChanged
    );
}
...
GUI_TL2_MediaTrack::initButtons()
{
    ...
    GUI_SvgButton* gearButton = new GUI_SvgButton(
        ":feather/command.svg", this, 18, 0, GUI_Style::TrackHeaderFg
    );
    ...
    GUI_SvgButton* thumbButton = new GUI_SvgButton(
        ":feather/image.svg", this, 18, 0, GUI_Style::TrackHeaderFg
    );
...
void
GUI_TL2_MediaTrack::onMediaRemoved(DS_NodePtr node, QString trackUuidStr, qlonglong startFrame, bool doSync)
{
    RLP_LOG_DEBUG("");
...
void
GUI_TL2_MediaTrack::onMediaEnabledChanged(QUuid nodeUuid, bool isEnabled)
{
    if (_itemMap.contains(nodeUuid))
    {
        // RLP_LOG_DEBUG(nodeUuid << isEnabled)
        _itemMap.value(nodeUuid)->setItemEnabled(isEnabled);
    }
}


void
GUI_TL2_MediaTrack::onMediaSelectionChanged(QUuid nodeUuid, bool isEnabled)
{
    if (_itemMap.contains(nodeUuid))
    {
        // RLP_LOG_DEBUG(nodeUuid << isEnabled)
        _itemMap.value(nodeUuid)->setSelected(isEnabled);
    }
}
