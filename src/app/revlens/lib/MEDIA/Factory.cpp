...
MEDIA_Factory::appendMedia(QVariantMap mediaIn, DS_Track* track)
{
    ...
    if (node != nullptr)
    {
        track->insertNode(node, track->getMaxFrameNum());
        return true;
    }
...
QVariantMap
MEDIA_Factory::identifyMedia2(QVariantMap mediaInput, int inodeDataType)
{
    // RLP_LOG_DEBUG(nodeDataType)

    DS_Node::NodeDataType nodeDataType = (DS_Node::NodeDataType)inodeDataType;
    ...
    for (int pi=0; pi != _mediaPlugins.size(); pi++)
    {
       ...
       // RLP_LOG_INFO("Attempting to identify media data using" << pl->name());
        ...
        if (mediaTypeList.contains(inodeDataType))
        {
...
MEDIA_Factory::onMediaDataReady(QString dataType, QVariantMap data)
{
    ...
    if (dataType == "video_source")
    {
        ...
        if (props->contains("load_uuid"))
        {
            QUuid loadUuid = props->value("load_uuid").toUuid();
        ...
        {
            RLP_LOG_ERROR("load_uuid missing from node")
            LOG_Logging::pprint(*props);
        }
    }
}
...
void
MEDIA_Factory::onLoadComplete(QUuid uuid)
{
    // RLP_LOG_DEBUG(uuid)
