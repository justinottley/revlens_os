...
MEDIA_CompositeNode::MEDIA_CompositeNode(
    ...
    DS_Node("MEDIA_Composite", properties)
{
    ...
    nodeIn->getPropertiesPtr()->insert("node.composite.enabled", true);
...
MEDIA_CompositeNode::addInput(DS_NodePtr nodeIn)
{
    ...
    _properties->insert("node.composite.count", (int)_inputs.size());
    ...
    emitDataReadyToGraph("media_input", nameInfo);
}


bool
MEDIA_CompositeNode::setInputEnabled(bool idx, bool isEnabled)
{
    RLP_LOG_DEBUG(idx << isEnabled)

    if (_inputs.size() < idx)
    {
        RLP_LOG_ERROR("Invalid input index, total inputs:" << _inputs.size())
        return false;
    }

    _inputs.at(idx)->setProperty("node.composite.enabled", isEnabled);
...
QVariantList
MEDIA_CompositeNode::getInputInfo()
{
    QVariantList result;
    for (int i=0; i != _inputs.size(); ++i)
    {
        DS_NodePtr inode = _inputs.at(i);
        QVariantMap nodeInfo;

        QVariantMap* props = inode->getPropertiesPtr();

        nodeInfo.insert("idx", i);
        nodeInfo.insert("enabled", props->value("node.composite.enabled"));
        nodeInfo.insert("uuid", inode->uuid().toString());
        if (props->contains("media.name.orig"))
        {
            nodeInfo.insert("name", props->value("media.name.orig"));
        } else
        {
            nodeInfo.insert("name", props->value("media.name"));
        }
...
MEDIA_CompositeNode::readVideo(DS_TimePtr timeInfo, DS_BufferPtr destFrame, bool postUpdate)
{
    ...
    if (inputCount == 1)
    {
        ...
        if (_graph->flags()->checkFlagByName("context"))
        {
            DS_Track* track = _properties->value("session.track").value<DS_Track*>();
            if (track->hasMetadataValue("context.greyscale"))
            {
                mbufm->insert("greyscale_mix", 1.0);
            } else
            {
                mbufm->insert("greyscale_mix", 0.0);
            }
        } else
        {
            mbufm->insert("greyscale_mix", 0.0);
    ...
    QVariantMap* mbufm = destFrame->getBufferMetadata();
    mbufm->insert("node.composite.count", inputCount);
    mbufm->insert("node.composite.idx", 0);
    // QList<DS_BufferPtr> buflist;

    for (int i=1; i != inputCount; ++i)
    {
        DS_NodePtr node = input(i);
        ...
        // Standard properties
        //
        bm->insert("node.composite.count", inputCount);
        bm->insert("node.composite.idx", i);
        bm->insert("media.name", node->getProperty<QString>("media.name"));
        bm->insert("media.metadata", node->getProperty<QVariantMap>("media.metadata"));
    ...
    input(0)->readVideo(timeInfo, destFrame, false); // postUpdate);
...
/*
    ...
    {
        QString compositeMode = _properties->value("display.composite_mode").toString();
        ...
        // Dispatch read to Composite Plugin
        //
        DISP_CompositePlugin* cplugin = _compositePlugins->value(compositeMode);
        cplugin->readVideo(this, timeInfo, destFrame);
    ...
    RLP_LOG_ERROR("Composite not handled, no data read for" << timeInfo->videoFrame())
...
void
MEDIA_CompositeNode::readVideo1(DS_TimePtr timeInfo, DS_BufferPtr destFrame, bool postUpdate)
{
    int inputCount = _inputs.size();
...
MEDIA_CompositeNode::event(QEvent* event)
{
    ...
    if (event->type() == MEDIA_FrameReadEventType) {
        ...
        {
            QVariantMap* auxdata = mainBuf->getBuffer(bi)->getBufferMetadata();
            ...
            if (mainBuf->getBuffer(bi)->isReady())
            {
                numReady++;
            }
        }
