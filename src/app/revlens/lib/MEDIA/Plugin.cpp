...
MEDIA_Plugin::getPreallocatedFrameBuffer(QVariantMap frameInfo)
{
    ...
    int bytePadding = frameInfo.value("bytePadding").toInt();

    int dataSize = (width * height * channelCount * pixelSize) + bytePadding;
    // int dataSize = frameInfo.value("byteCount").toInt();
