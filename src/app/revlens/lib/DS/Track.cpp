...
void
DS_Track::setMetadataValue(QString key, QVariant value)
{
...
bool
DS_Track::hasMetadataValue(QString key)
{
    QMutexLocker l(_lock);
...
QVariant
DS_Track::metadataValue(QString key)
{
    QMutexLocker l(_lock);

    return _metadata.value(key);
}


void
DS_Track::removeMetadataValue(QString key)
{
    QMutexLocker l(_lock);
...
DS_NodePtr
DS_Track::getNodeByUuid(QString uuidStr)
{
    QUuid u(uuidStr);
    if (_mediaMap.contains(u))
    {
        return _mediaMap[u].node;
    }
...
QVariantList
DS_Track::getFrameRange()
{
    qlonglong minFrame = INT_MAX;
    qlonglong maxFrame = 0;
    ...
    if (fl.size() >= 2)
    {
        result.push_back(fl.at(0));
        result.push_back(fl.at(fl.size() - 1));
    }
...
DS_Track::updateMediaIndex2(QUuid atNodeUuid, qlonglong newLength)
{
    ...
    if ((nodeIndex > -1) && (nodeIndex < currNodeList.size()))
    {
        ...
        for (int ci = nodeIndex; ci != currNodeList.size(); ++ci)
        {
            ...
            nri.node->setProperty<qlonglong>("session.frame", atFrame);
...
void
DS_Track::updateMediaIndexInternal(
    DS_NodePtr media,
    qlonglong frameNum,
    ...
    QVariantList* mediaChangeList)
{
    ...
    QVariantMap cInfo;
    cInfo.insert("node_uuid", mediaUuid.toString());
    cInfo.insert("frame", nfi.frame);
    cInfo.insert("frame_count", nfi.frameCount);
...
DS_Track::insertNode(DS_NodePtr media, qlonglong frameNum)
{
    ...
    if (_mediaMap.contains(mediaUuid))
    {
        ...
        RLP_LOG_ERROR("Node already present in track, aborting")
        return false;
    ...
    } else
    {
        ...
        if (mediaFound)
        {
            ...
            // get the index of the node at the insertion frame
            for (int i=0; i != currNodeList.size(); ++i)
            {
                QVariantMap cNodeInfo = currNodeList.at(i).toMap();
                qlonglong nodeFrame = cNodeInfo.value("frame").toLongLong();
                
                if (nodeFrame == frameNum)
                {
                    atNodeIndex = i;
                    break;
                }
            ...
            if (atNodeIndex == -1)
            {
                RLP_LOG_ERROR("cannot insert, invalid insertion index")
                return false;
            }
            ...
            for (qlonglong f = frameNum; f <= _session->frameCount(); ++f)
            {
                if (!_frameNumMap.contains(f))
                {
                    continue;
            ...
            //
            // Setup inserting media node
            //

            updateMediaIndexInternal(media, frameNum, frameCount, &mediaChangeList);

            qlonglong atFrame = frameNum + frameCount;

            // reinsert everything at insertion index
            //
            for (int i=atNodeIndex; i != currNodeList.size(); ++i)
            {

                QVariantMap cNodeInfo = currNodeList.at(i).toMap();
                QUuid cNodeUuid = QUuid(cNodeInfo.value("node_uuid").toString());

                if (!_mediaMap.contains(cNodeUuid))
                {
                    RLP_LOG_WARN("nodeUuid not found in mediamap:" << cNodeUuid)
                    continue;
                }

                NodeFrameInfo nri = _mediaMap.value(cNodeUuid);
                updateMediaIndexInternal(nri.node, atFrame, nri.frameCount, &mediaChangeList);
                atFrame = atFrame + nri.frameCount;
            }
            ...
            QVariantMap evtInfo;
            evtInfo.insert("media", mediaChangeList);
            evtInfo.insert("type", "insert_move");
...
bool
DS_Track::insertNodeAtIndex(DS_NodePtr node, int index)
{
    RLP_LOG_DEBUG(index)

    QVariantList currNodeList = getNodeFrameList();

    int numNodes = currNodeList.size();
    if (index >= numNodes)
    {
        RLP_LOG_WARN("index" << index << "larger than number of items:" << numNodes)
        return false;
    }
...
bool
DS_Track::moveNodeToFrame(DS_NodePtr node, int nodeIdx, qlonglong frameNum, int destIdx)
{
    ...
    if (!_mediaMap.contains(nodeUuid))
    {
        RLP_LOG_ERROR("Node not present in this track with UUID" << nodeUuid)
        return false;
    }


    qlonglong currNodeFrame = node->getProperty<qlonglong>("session.frame");

    qlonglong startFrame = currNodeFrame; // earlier frame to later frame
    if (frameNum < startFrame)
    {
        startFrame = frameNum; // later frame to earlier frame
    }


    QVariantList currNodeList = getNodeFrameList();

    // Remove everything in the frame index starting at lower frame of the source or dest
    //
    for (qlonglong f = startFrame; f <= _session->frameCount(); ++f)
    {
        if (!_frameNumMap.contains(f))
    ...
    if (currNodeFrame > frameNum)
    {
        // later frame to earlier frame, drop node into new spot, pull all media that occurs
        // after the requested node back
        //

        // Update the frame for the requested node
        //

        NodeFrameInfo nri = _mediaMap.value(nodeUuid);
        updateMediaIndexInternal(nri.node, frameNum, nri.frameCount, &mediaChangeList);

        qlonglong atFrame = frameNum + nri.frameCount;

        // pull all media back
        //
        for (int i=destIdx; i != currNodeList.size(); ++i)
        {
            QVariantMap cNodeInfo = currNodeList.at(i).toMap();
            QUuid cNodeUuid = QUuid(cNodeInfo.value("node_uuid").toString());

            if (nodeUuid == cNodeUuid)
            {
                // RLP_LOG_DEBUG("Found moving node, skipping")
                continue;
            }
    ...
    {
        // Earlier frame to later frame
        //

        // first, pull everything between earlier frame and dest frame back
        //
        //
        qlonglong atFrame = currNodeFrame;

        for (int i=nodeIdx + 1; i != destIdx; ++i)
        {
            QVariantMap cNodeInfo = currNodeList.at(i).toMap();
            QUuid cNodeUuid = QUuid(cNodeInfo.value("node_uuid").toString());
        ...
        //
        // Insert moving node at this position
        //
        ...
        //
        // Insert everything after dest frame
        //
        for (int i = destIdx; i != currNodeList.size(); ++i)
        {
            QVariantMap cNodeInfo = currNodeList.at(i).toMap();
...
DS_Track::resetNodeEnabled()
{
    ...
    for (it = _mediaMap.begin(); it != _mediaMap.end(); ++it)
    {
        ...
        if (!it->node)
        {
            RLP_LOG_WARN("Invalid node found, skipping")
            continue;
        }

        it->enabled = true;
        QUuid nodeUuid = it->node->graph()->uuid();
        emit mediaEnabledChanged(nodeUuid, true);
        emit mediaSelectionChanged(nodeUuid, false);
    }
}
...
DS_Track::onMediaDataReady(QString dataType, QVariantMap data)
{
    ...
    {
        emit trackDataChanged("media_input_changed", _uuid.toString(), data);
    }

    // TODO FIXME: creating a signal with the data does not pass through
    // binding.. why!?!??!
    emit mediaDataReady(dataType, data);
}
