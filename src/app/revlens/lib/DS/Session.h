...
signals:
    ...
    void propertyChanged(QString key, QVariant val);

    // re-emitted from track
    void mediaAdded(DS_NodePtr node, QString uuidStr, qlonglong frameNum, qlonglong mediaFrameCount);

    void mediaRemoved(DS_NodePtr node, QString trackUuidStr, qlonglong atFrame, bool doSync);
...
public slots:
    ...
    static QString cacheDir(QString subdir = "");
    QString siteCacheDir();

    QVariantMap properties() { return _properties; }
    void setProperty(QString key, QVariant val);
    QVariant getProperty(QString key) { return _properties.value(key); }
    bool hasProperty(QString key) { return _properties.contains(key); }
    ...
    DS_TrackPtr getTrackByName(QString name);
    QVariantList getTracksByType(int trackType);
    bool hasTrackType(int trackType) { return _trackMap.contains(trackType); }
    ...
    DS_TrackPtr getSoloedTrackByType(int trackType);
    ...
    void onMediaRemoved(DS_NodePtr node, QString trackUuidStr, qlonglong atFrame, bool doSync);
