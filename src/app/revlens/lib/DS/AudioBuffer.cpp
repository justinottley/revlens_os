...
DS_AudioBuffer::reallocate()
{
    ...
    int bytesPerSample = _bufferMetadata.value("audio.output.bit_depth").value<int>();
    int channels = _bufferMetadata.value("audio.output.channel_count").value<int>();
...
{
    _bufferMetadata.insert("audio.output.bit_depth", bitDepth);
    _bufferMetadata.insert("audio.output.channel_count", channels);
