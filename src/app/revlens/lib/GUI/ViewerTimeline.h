...
public:
    ...
    QRectF timelineBoundingRect(); // different method name here to not be confused with the regular GUI method
    void paintGLPostDraw(GUI_Painter* painter, const QVariantMap& metadata);

    qlonglong getFrameNum(int xpos);

    void setFrameInOut(qlonglong frameIn, qlonglong frameOut);
    void setFrameNumberStyle(int fnumStyle) { _fnumStyle = (FrameNumberStyle)fnumStyle; }

    
protected:
    FrameNumberStyle _fnumStyle;
