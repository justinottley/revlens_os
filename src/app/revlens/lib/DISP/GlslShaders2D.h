...
static const char* vertex_shader_2D = R"(
        ...
        void main(void)
        {
            ...
            scaling = scaling * vec2(relZoom);
            scaling = scaling * vec2(1.0 + zoom);
...
static const char* fragment_shader_2D = R"(
        ...
        vec3 LinearRGB_to_sRGB(vec3 linearRGB) {
            return vec3(
                sRGB_gamma_correction(linearRGB.r),
                sRGB_gamma_correction(linearRGB.g),
                sRGB_gamma_correction(linearRGB.b)
            );
        }
        ...
        vec4 greyscaleMain(vec4 color, float gmix) {
        ...
        void main()
        {
            ...
            if (do_grayscale == 1)
            {
                color = greyscaleMain(color, greyscale_mix);
            }

            if (colspace_linear == 1)
            {
                // A simple gamma correction
