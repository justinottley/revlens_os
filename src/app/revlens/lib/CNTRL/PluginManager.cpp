...
bool
CNTRL_PluginManager::registerCompositePlugin(DISP_CompositePlugin* plugin)
{
    RLP_LOG_DEBUG(plugin->name())

    return _controller->getVideoManager()->registerCompositePlugin(plugin);
}
...
{
    MEDIA_Factory* factory = _controller->getMediaManager()->getMediaFactory();
    MEDIA_Plugin* mplugin = factory->getMediaPluginByName(name);
