...
signals:
    ...
    void serviceMessageReady(QVariantMap msg);


public slots:
    
    void startIOService(int port);
    ...
    // TODO FIXME DEPRECATE, SWITCH TO NAMED CLIENT
    // send to SITE Client
    void siteClientSendCall(
        QString method,
        QVariantList args=QVariantList(),
        QVariantMap kwargs=QVariantMap(),
        QVariantMap skwargs=QVariantMap(),
        CoreNet_RPCFuture* callback=nullptr
    );


    void sendCallToClient(QVariantMap data) { _wsServer->sendCallToClient(data); }

    void sendCallToNamedClient(
        QString clientName,
        QString method,
        QVariantList args=QVariantList(),
        QVariantMap kwargs=QVariantMap(),
...
public slots:
    ...
    void registerNamedClient(QString name, CoreNet_WebSocketClient* client);
    CoreNet_WebSocketClient* getWsClient(QString clientName);
...
private:
    ...
    QHash<QString, CoreNet_WebSocketClient*> _wsClientMap;
