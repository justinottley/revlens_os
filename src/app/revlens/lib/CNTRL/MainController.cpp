...
CNTRL_MainController::CNTRL_MainController():
    DS_ControllerBase()
{
    CoreDs_Flags::registerFlag("valid");
    CoreDs_Flags::registerFlag("context"); // TODO FIXME: workflow specific, move
    CoreDs_Flags::registerFlag("context_main"); // TODO FIXME: workflow specific, move
    ...
    _playbackMode->setSession(_session);
    ...
    _pluginManager->registerVideoProbe(new CNTRL_FrameProbe());
...
CNTRL_MainController::initConnections()
{
    ...
    connect(
        ...
        _videoManager,
        &CNTRL_Video::onFrameCountChanged
    );
...
CNTRL_MainController::initCommands()
{
    ...
    registerCommand(new CNTRL_RecenterCommand(this));
}
...
CNTRL_MainController::setSession(QString name)
{
    ...
    {
        _session = session;
        _playbackMode->setSession(session);
...
{
    if (_playbackState == CNTRL_MainController::PLAYSTATE_PAUSED)
    {
        play();
...
{
    _playbackState = CNTRL_MainController::PLAYSTATE_PLAYING;
...
CNTRL_MainController::pause(qlonglong frameNum, bool emitAction)
{
    ...
    _playbackState = CNTRL_MainController::PLAYSTATE_PAUSED;
...
QVariantMap
CNTRL_MainController::getDisplayMetadata()
{
    QVariantMap result;

    QList<DISP_GLView*> displayList = _videoManager->getDisplays();

    result.insert("disp.count", displayList.size());

    for (int i=0; i != displayList.size(); ++i)
    {
        QVariantMap* dispMd = displayList.at(i)->getDisplayMetadata();
        QVariantMap::iterator dit;
        for (dit = dispMd->begin(); dit != dispMd->end(); ++dit)
        {
            QString key = QString("disp.%1.%2").arg(i).arg(dit.key());
            QVariant val = dit.value();
...
QVariantList
CNTRL_MainController::getAllVideoSources()
{
    return CNTRL_Video::_VIDEO_SOURCES;
}
...
CNTRL_MainController::fullRefresh()
{
    ...
    // WARNING - can result in crashes on Linux
...
CNTRL_MainController::onTrackDataChanged(QString eventName, QString trackUuidStr, QVariantMap trackData)
{
    ...
    if ((eventName == "set_enabled") || (eventName == "media_updated"))
    {
        fullRefresh();
    }
}
...
{
    RLP_LOG_INFO("Attempting immediate shutdown")

    // Calling this causes hang on quit
    // PY_Interp::call("revlens.service.iosched.force_quit");

    // Calling exit() takes longer than it should on linux
    // _exit(0);
