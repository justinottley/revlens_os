...
def start_launcher(global_dir):

    launcher_bin = os.path.join(global_dir, 'bin', 'rlp_launcher')
    if not os.path.isfile(launcher_bin): # macOS app bundle?
        global_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(global_dir))))
        launcher_bin = os.path.join(global_dir, 'bin', 'rlp_launcher.bin')

    print('Attempting to run: {}'.format(launcher_bin))
    os.execvpe(launcher_bin, [launcher_bin], os.environ)
...
def main():
    ...
    if '--sg' in sys.argv:
        sgArgIdx = sys.argv.index('--sg')
        sgConfigName = sys.argv[sgArgIdx + 1]
        os.environ['RLP_SG_CONFIG_TYPE'] = sgConfigName

    if '--launcher' in sys.argv:
        start_launcher(global_dir)
        return
