...
revlens_root = '/disk1/scratch/jottley/dev/revlens_root'
superproject_root = 'revlens_public'
...
if platform.system() == 'Darwin':
    revlens_root = '/Users/jottley/dev/revlens_root'
...
def dist_revlens(dist_ver):
    ...
    dist_revsix_topdir = os.path.join(dist_topdir, 'revlens_public')

    dist_build_dir = os.path.join(top_dir, 'dist', 'revlens', dist_ver)
    dist_revsix_dir = os.path.join(dist_build_dir, 'revlens_public')
    ...
    if os.name == 'nt':
        ...
        bat_text += r'%~dp0\..\thirdbase\25_04\Windows-AMD64-10\Python\{}\python.exe %~dp0\revlens_public\inst\Windows\revlens\bin\revlens.pyc %*'.format(PYVER)
        ...
        bat_text += 'set RLP_FS_ROOT=R:\n\n'
        bat_text += r'%~dp0\..\thirdbase\25_04\Windows-AMD64-10\Python\{}\python.exe %~dp0\revlens_public\inst\Windows\prod\bin\rlp_launcher.pyc %*'.format(PYVER)
        ...
        tr_bat_text += r'%~dp0\thirdbase\25_04\Windows-AMD64-10\python\{}\python.exe %~dp0\{}\revlens_public\inst\{}\revlens\bin\revlens.pyc %*'.format(PYVER, dist_ver, _get_platform_str())
        ...
        env_start_text += 'export SR=$RR/revlens_public/inst/{}\n'.format(_get_platform_str())
        ...
        # env_start_text += 'export REQUESTS_CA_BUNDLE=$RR/revlens_public/inst/Linux-x86_64/web/etc/cert.pem\n'
        ...
        # sh_text += '. $SCRIPT_DIR/revlens_public/inst/{}/revlens/bin/env_start.sh\n\n'.format(_get_platform_str())

        r_sh_text = 'export PATH=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python' + PYVER + '/bin:$PATH\n'
        r_sh_text += 'export LD_LIBRARY_PATH=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/lib\n'
        r_sh_text += 'export PYTHONHOME=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '\n'
        r_sh_text += '$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/bin/python3 $SCRIPT_DIR/revlens_public/inst/{}/extrevlens/bin/revlens $@'.format(_get_platform_str())
        
        rl_sh_text = 'export PATH=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/bin:$PATH\n'
        rl_sh_text += 'export LD_LIBRARY_PATH=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/lib\n'
        rl_sh_text += 'export PYTHONHOME=$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER+ '\n'
        rl_sh_text += '$SCRIPT_DIR/../thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/bin/python3 $SCRIPT_DIR/revlens_public/inst/{}/prod/bin/rlp_launcher $@'.format(_get_platform_str())

        tr_sh_text = '\n# Control login screen on launch\n'
        tr_sh_text += '# export RLP_NO_LOGIN=1\n\n'
        tr_sh_text += 'export PATH=$SCRIPT_DIR/thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/bin:$PATH\n'
        tr_sh_text += 'export LD_LIBRARY_PATH=$SCRIPT_DIR/thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/lib\n'
        tr_sh_text += 'export PYTHONHOME=$SCRIPT_DIR/thirdbase/25_04/Linux-x86_64/python/' + PYVER + '\n'
        tr_sh_text += '$SCRIPT_DIR/thirdbase/25_04/Linux-x86_64/python/' + PYVER + '/bin/python3 $SCRIPT_DIR/{}/revlens_public/inst/{}/extrevlens/bin/revlens $@'.format(dist_ver, _get_platform_str())
...
def main():
    ...
    parser.add_argument('--root', dest='top_dir', default=revlens_root, help='root of dist')
    ...
    if args.dist_ver:
        dist_ver = args.dist_ver[0]

    else:
        platform_reldir = os.path.join(args.top_dir, superproject_root, 'inst', _get_platform_str())
