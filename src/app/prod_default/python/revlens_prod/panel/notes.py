...
class Note(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent, username, user_info, text, media_info, media, create_time=None, thumb_data=None, note_id=None):
        ...
        # TODO FIXME: switching from TextEdit to Label in an attempt to improve
        # instability / crashing issues with this tool

        # self.note_text = RlpGui.GUI_TextEdit(self, 200, 110)
        # self.note_text.textArea().setReadOnly(True)
        self.note_text = RlpGui.GUI_Label(self, '')
    ...
    def init_image_data(self):
        ...
        if self.media:
            ...
            imageList = preview_node.makePreviewAllBuffers(int(self.frame))
            if len(imageList) == 2:

                import rlplug_annotate
                # annImage = rlplug_annotate.RLANN_CNTRL_DRAWCONTROLLER_OBJ.extractAnnotation(
                ...
                print('GOT ANNOTATION EXTRACTED IMAGE')
                
                self._init_thumb(flattenedImage)
    ...
    def _setWidth(self, width):
        ...
        self.note_text.setWidth(max_text_width)

        # Disabled for Label to help address instability issues, reenable for TextEdit
    ...
    def on_frame_button_pressed(self, md=None):
        revlens.cmds.goto_frame(int(self.media_info['session.frame']))
...
class NotesPanel(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent):
        ...
        self.new_note_textarea = RlpGui.GUI_TextEdit(self, 200, 40)
        ...
        self.email_button = RlpGui.GUI_SvgButton(
            ":feather/mail.svg",
            self,
            20
        )
        ...
        self.email_button.setToolTipText("Email Notes")
        self.email_button.buttonPressed.connect(self.on_export_requested)

        self.toolbar_layout.setSpacing(10)
        self.toolbar_layout.addItem(self.refresh_button, 3)
        # self.toolbar_layout.addSpacer(10)
        self.toolbar_layout.addItem(self.email_button, 4)
        self.toolbar_layout.addItem(self.export_button, 3)
        # self.toolbar_layout.addDivider(20, 25, 0)
    ...
    def _init_connections(self):
        ...
        session.mediaAdded.connect(self._init_notes_from_media)
    ...
    def _init_notes_from_media(self, media_in, *args):
        ...
        for note_entry in note_list:
            ...
            sessionFrame = note_entry['frame'] + props['session.frame']
            media_info = {
                'session.frame': sessionFrame,
                'media.frame': note_entry['frame'],
            ...
            new_note_list.append(new_note.to_dict())

        media_in.insertProperty('media.notes', new_note_list)
    ...
    def _sync_note(self, media_props, media_notes):
        ...
        print('sync_note track: {} media: {} notes: {}'.format(
            track_uuid, media_uuid, len(media_notes)
        ))
        ...
        revlens.CNTRL.emitNoticeEvent('notes_changed', kwargs['update_info'])
        revlens.CNTRL.emitSyncAction('mediaUpdate', kwargs)


    def add_note(self, media_info,
                       note_text,
                       media,
                       create_time=None,
                       username=None,
        ...
        if not user_info:
            ...
            if 'user.username' in ag:
                username = ag['user.username']
                user_info = {
                    'first_name': ag['user.first_name'],
                    'last_name': ag['user.last_name']
                }
        ...
        self.note_list_sa.setMaxChildHeight(note_height)
        self.note_layout.setHeight(note_height)
    ...
    def new_note_requested(self, val):
        ...
        frame = revlens.cmds.get_current_frame_num()
        
        print('NEW NOTE REQUESTED ON FRAME {}'.format(frame))
        ...
        curr_notes.append(new_note.to_dict())
    ...
    def on_remove_note(self, note, md=None):
        ...
        def _on_delete(note, md):

            # remove from properties
            note_id = note.note_id
            note_frame = note.frame
            props = note.media.getProperties()

            new_notes = []
            for media_note_info in props.get('media.notes', []):
                if media_note_info['note_id'] != note_id:
                    new_notes.append(media_note_info)

            note.media.insertProperty('media.notes', new_notes)

            self.notes.remove(note)
            self.note_layout.removeItem(note)
            self.note_layout.update()

            # TODO FIXME: proper API for this, maybe part of insertProperty()?
            evt_info = {
                'media_type': 'media_property',
                'track_uuid': props['session.track'].uuid().toString(),
                'media_uuid': props['graph.uuid'].toString(),
                'prop_key': 'media.notes',
                'prop_val': new_notes
            }

            revlens.CNTRL.emitNoticeEvent('notes_changed', evt_info)



        diag.accept.connect(functools.partial(_on_delete, note))
    ...
    def on_toggle_grab_annotations(self, md):
        ...
        isToggled = not md['toggled']

        self.annotation_button.setOutlined(isToggled)
        self.annotation_button.setToggled(isToggled)
        self.grab_annotations = isToggled
    
        if isToggled:
            self.annotation_button.setBgColor(QtGui.QColor(180, 180, 180))

        else:
            self.annotation_button.setBgColor(QtGui.QColor(80, 80, 80))

        self.annotation_button.update()
    '''


def create(parent):
    return NotesPanel(parent)