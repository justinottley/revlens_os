...
def load_shots(track, selList, atFrame=None):

    pd = RlpUtil.APPGLOBALS.value('prod_data')
    showDataObj = pd.getShowData()
    cbMediaDataFormatter = showDataObj.formatter('code')

    if atFrame is None:
        atFrame = track.getMaxFrameNum() + 1

    def _mediaListReady(mediaList):

        loadMediaList = []
        for mediaInfo in mediaList:

            mediaName, mediaMd = cbMediaDataFormatter(mediaInfo, nameKey='pri')
            loadMediaList.append({
    ...
    seq = selList[0]['sg_sequence']['name']
    shotList = []
    for selEntry in selList:

        shotCode = selEntry['code']
        if '/' in shotCode:
            shotCode = shotCode.split('/')[1]

        shotList.append(shotCode)

    shotCls = pd.getProdDataClass('shot')
    shotCls.requestShots(
        _mediaListReady,
         showDataObj.projectEntity['code'],
        seq,
        shotList)
