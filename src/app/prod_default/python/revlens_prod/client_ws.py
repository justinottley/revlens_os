...
from rlp.core.service.client import RlpIOServiceClient
...
class SiteWsClient(RlpQtWsClient):
    ...
    def _init_ioclient(self, name):
        ...
        if name in self._ioclients:
            return

        from rlp.core.service.client import RlpIOServiceClient, RlpWsProxyIOServiceClient

        ioServiceClientCls = RlpIOServiceClient
        ag = RlpUtil.APPGLOBALS.globals()
        if ag['app.platform'] == 'ios':
            ioServiceClientCls = RlpWsProxyIOServiceClient

        ioname = 'io{}'.format(name)
        self._ioclients[name] = ioServiceClientCls(ioname)

        print('Initialized IOClient: {} - {}'.format(name, ioServiceClientCls.__name__))
