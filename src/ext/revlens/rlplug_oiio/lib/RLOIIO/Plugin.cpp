...
RLOIIO_Plugin::identifyMedia(QVariantMap mediaInput, DS_Node::NodeDataType dataType)
{
    ...
    //QStringList ignoreList = {"exr", "jpg", "jpeg", "png", "mov", "mp4", "mj2"};
    QStringList ignoreList = {"exr", "mov", "mp4", "mj2"};

    QStringList::iterator it;
    for (it = ignoreList.begin(); it != ignoreList.end(); ++it)
    {
        if (videoPath.endsWith(*it))
        {
            RLP_LOG_DEBUG("Skipping:" << *it)
            return result;
        }
    }
    ...
    */
    ...
    SEQ_SequencePtr seq = nullptr;
    if (mediaInput.contains("media.sequence"))
    {
        RLP_LOG_DEBUG("Using user-provided sequence")
        SEQ_Sequence* sseq = mediaInput.value("media.sequence").value<SEQ_Sequence*>();

        // TEMP WORKAROUND FOR SequencePtr handling, to not deallocate on scope exit
        seq = SEQ_SequencePtr(sseq);
        _tempSeq.append(seq);

        RLP_LOG_WARN("Keeping reference to Sequence object for reference counting")

    } else
    {
        seq = SEQ_Util::getSequenceFromTemplateString(videoPath); // .get();
    }
