//
//  Copyright (c) 2014 Justin Ottley. All rights reserved.
//

#ifndef EXTREVLENS_RLOIIO_VIDEOREADER_H
#define EXTREVLENS_RLOIIO_VIDEOREADER_H

#include "RlpExtrevlens/RLOIIO/Global.h"

#include "RlpCore/SEQ/Sequence.h"
#include "RlpRevlens/DS/Node.h"




class EXTREVLENS_RLOIIO_API RLOIIO_VideoReader : public DS_Node {
    
public:
    RLP_DEFINE_LOGGER

    RLOIIO_VideoReader(QVariantMap* properties);
    
    void readVideo(
        DS_TimePtr timeInfo,
        DS_BufferPtr destFrame,
        bool postUpdate = true);
    
    
    void scheduleReadTask(
        DS_TimePtr timeInfo,
        DS_BufferPtr destFrame,
        DS_Node::NodeDataType dataType,
        bool optional = false);
    
private:

    // TODO FIXME: refcounting issues with this and python
    SEQ_SequencePtr _seq;
    SEQ_Sequence* _sseq;

};

#endif

