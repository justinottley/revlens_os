...
from . import PaintBrushButton, EraserBrushButton
from . import ICON_SIZE


class AnnotationToolbarPanel(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent):
        ...
        self.brush_none_button = RlpGui.GUI_IconButton("", self, ICON_SIZE - 5, 4)
        self.brush_none_icon = RlpGui.GUI_SvgIcon(
            ":misc/cursor.svg", self.brush_none_button, ICON_SIZE - 2, 4, QtGui.QColor(180, 180, 180))
        self.brush_none_button.setSvgIcon(self.brush_none_icon)
        self.brush_none_button.svgIcon().setPos(3, 3)

        self.brush_none_button.setToolTipText('Curstor / No Brush')
        self.brush_none_button.setToggled(True)
        ...
        self.brush_sizecol_button.setValue(1)
        ...
        # DISABLED, causing python garbage collection resulting in c++ destructors
        # INCOMPLETE
        #
        # self.brush_settings_button = RlpGui.GUI_SvgButton(':misc/creative-process.svg', self, ICON_SIZE)
        # self.brush_settings_button.setToolTipText('Toggle Brush Settings')
        # self.brush_settings = BrushToolSettings(viewer_toolbar.parentItem().view())
        # self.brush_settings.setVisible(False)

        self.layout.setSpacing(5)
        # self.layout.addDivider(20, ICON_SIZE + 5, 0)
        self.layout.addItem(self.brush_none_button, 2)
        self.layout.addItem(self.brush_paint_button, 2)
        self.layout.addItem(self.brush_eraser_button, 2)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.text_button, 4)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.colour_picker_button, 4)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.brush_sizecol_button, 3)
        # self.layout.addSpacer(5)
        # self.layout.addItem(self.brush_settings_button, 4)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.paper_trace_button, 4)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.undo_button, 4)
        # self.layout.addSpacer(5)
        self.layout.addItem(self.redo_button, 4)
        #self.layout.addSpacer(5)

        self.brush_none_button.buttonPressed.connect(self.on_brush_none_selected)
        self.brush_none_icon.buttonPressed.connect(self.on_brush_none_selected)
        ...
        # self.brush_settings_button.buttonPressed.connect(self.on_brush_settings_toggled)
        ...
        self.draw_controller.brushInfoChanged.connect(self.on_brush_info_changed)
    ...
    def reset_all_brushes(self):
        ...
        self.brush_none_button.setToggled(False)
        self.brush_none_button.icon().setOutlined(False)
        self.brush_none_button.icon().update()

        self.brush_paint_button.setToggled(False)
        self.brush_paint_button.update()
        self.brush_eraser_button.setToggled(False)
        self.brush_eraser_button.update()
    ...
    def on_brush_none_selected(self, md):

        if self._destructed:
            return

        self.reset_all_brushes()

        self.brush_none_button.setToggled(True)

        self.draw_controller.setBrush('NONE', '')
    ...
    def on_brush_eraser_selected(self, md):
        ...
        self.draw_controller.setBrush('MyPaintEraser', '')
    ...
    def on_text_selected(self, md):
        ...
        self.text_button.setToggled(True)
        self.text_button.icon().setOutlined(True)
        self.text_button.icon().update()
    ...
    def on_brush_size_changed(self, valInfo):
        ...
        self.draw_controller.setBrushSettings({'width': value}, "", True)
    ...
    def on_brush_colour_changed(self, colInfo):
        ...
        self.draw_controller.setBrushSettings({'col': colInfo}, "", True)
    ...
    def on_brush_settings_toggled(self, md):

        val = not md['toggled']
        self.brush_settings_button.setToggled(val)
        self.brush_settings_button.icon().setOutlined(val)
        self.brush_settings_button.icon().update()

        self.brush_settings.setVisible(val)
        print('brush settings')
        print(md)
...
def createToolbar(parent):
    return AnnotationToolbarPanel(parent)