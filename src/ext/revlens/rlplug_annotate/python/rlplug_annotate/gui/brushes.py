
import functools

from rlp import QtCore
from rlp import QtGui
import rlp.gui as RlpGui

import rlplug_annotate

_PANELS = []


class BrushGroupPane(RlpGui.GUI_ItemBase):

    def __init__(self, parent, name):
        RlpGui.GUI_ItemBase.__init__(self, parent)

        self.brushSelected = QtCore.PY_Signal(self)

        self.mp_handler = parent.mp_handler
        self.name = name

        self.layout = RlpGui.GUI_FlowLayout(self)
        self.layout.setOutlined(False)
        self.layout.setSpacing(10)

        self._thumbs = []
        self._init()

        self.onParentSizeChangedItem(parent.width(), parent.height())


    @property
    def drawController(self):
        return rlplug_annotate.RLANN_CNTRL_DRAWCONTROLLER_OBJ


    def _init(self):

        brushes = self.mp_handler.getBrushes(self.name)
        for brush_name in brushes:
            brush_path = ':brushes/{}/{}_prev.png'.format(self.name, brush_name)

            ic = RlpGui.GUI_IconButton(
                brush_path, self, 64, 4)

            ic.setMetadata('brush_name', brush_name)
            ic.setMetadata('category', self.name)
            ic.buttonPressed.connect(self.onBrushSelected)

            self.layout.addItem(ic)
            self._thumbs.append(ic)

        self.layout.updateItems()


    def onParentSizeChangedItem(self, width, height):

        if not self.isVisible():
            return

        self.setWidth(width - 20)
        self.setHeight(height - 130) #  - 50)


    def onBrushSelected(self, md):
        
        
        self.drawController.setBrushSettings({
            'brush_name': md['brush_name'],
            'brush_category': self.name
        }, "", True)

        self.brushSelected.emit(md)


class BrushPaneArea(RlpGui.GUI_ItemBase):

    def __init__(self, parent):
        RlpGui.GUI_ItemBase.__init__(self, parent)

        self.brushSelected = QtCore.PY_Signal(self)

        self.mp_handler = parent.mp_handler

        self.panes = {}

        self.onParentSizeChangedItem(parent.width(), parent.height())


    def onParentSizeChangedItem(self, width, height):
        self.setWidth(width)
        self.setHeight(height)

    def onBrushSelected(self, md):
        self.brushSelected.emit(md)

    def updateBrushes(self, bcat):

        bcat = bcat.lower()

        if bcat not in self.panes:
            self.panes[bcat] = BrushGroupPane(self, bcat)
            self.panes[bcat].brushSelected.connect(self.onBrushSelected)

        for pane in self.panes.values():
            pane.hideItem()

        self.panes[bcat].showItem()




class BrushPanel(RlpGui.GUI_ItemBase):

    def __init__(self, parent):
        RlpGui.GUI_ItemBase.__init__(self, parent)

        self.colours = {
            'active': {
                'fgCol': QtGui.QColor(80, 80, 120),
                'bgCol': QtGui.QColor(60, 60, 100)
            },
            'inactive': {
                'fgCol': QtGui.QColor(80, 80, 80),
                'bgCol': QtGui.QColor(60, 60, 60)
            }
        }

        self.brush_categories = self.mp_handler.getBrushCategories()

        self.cat_widgets = {}
        self.toolbar_layout = RlpGui.GUI_HLayout(self)
        self.toolbar_layout.addSpacer(10)

        self.pane_area = BrushPaneArea(self)

        self.layout = RlpGui.GUI_VLayout(self)
        self.layout.addItem(self.toolbar_layout, 0)
        self.layout.addSpacer(10)
        self.layout.addItem(self.pane_area, 0)

        for bcat in self.brush_categories:

            if bcat == 'favorites':
                continue

            button = RlpGui.GUI_IconButton('', self, 20, 4)
            button.setMetadata('category', bcat)
            button.setText(bcat.capitalize())
            button.setOutlined(True)
            button.setOutlinedBgColour(self.colours['inactive']['bgCol'])
            button.setHoverColour(self.colours['inactive']['fgCol'])

            button.buttonPressed.connect(self.onCategoryRequested)

            self.cat_widgets[bcat] = {
                'button': button
            }

            self.toolbar_layout.addItem(button, 0) # 15)

        self.onCategoryRequested({'category':'classic'})

        self.onParentSizeChangedItem(parent.width(), parent.height())


    @property
    def drawController(self):
        return rlplug_annotate.RLANN_CNTRL_DRAWCONTROLLER_OBJ


    @property
    def mp_handler(self):
        return self.drawController.mypaintHandler()

    def onParentSizeChangedItem(self, width, height):

        self.setWidth(width)
        self.setHeight(height)


    def onCategoryRequested(self, md):

        category = md['category']
        for cat in self.cat_widgets:
            cat_button = self.cat_widgets[cat]['button']
            if cat == category:
                cat_button.setOutlinedBgColour(
                    self.colours['active']['bgCol']
                )
                cat_button.setHoverColour(
                    self.colours['active']['fgCol']
                )

            else:
                cat_button.setOutlinedBgColour(
                    self.colours['inactive']['bgCol']
                )
                cat_button.setHoverColour(
                    self.colours['inactive']['fgCol']
                )

            cat_button.update()

        self.pane_area.updateBrushes(md['category'])


def create(parent):
    return BrushPanel(parent)
    # global _PANELS
    # _PANELS.append(brush_panel)

    # return brush_panel

