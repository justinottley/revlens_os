...
def startup():
    ...
    def _on_arrangement_loaded():
        ...
        if m_create_track in revlens.gui.menu._UI and m_create_track_draw not in revlens.gui.menu._UI:
            act_create_annotation_track = revlens.gui.menu._UI[m_create_track].addAction(
                'Annotation')

            act_create_annotation_track.triggered.connect(_create_track)
            revlens.gui.menu._UI['menu.file.create_track.annotation'] = act_create_annotation_track
    ...
    revlens.plugin.register(
        ...
        {
            'name': 'Annotate',
            'callback_ns': 'rlplug_annotate.gui.toolbar',
            'alignment': 'bottom'
        }
    )
