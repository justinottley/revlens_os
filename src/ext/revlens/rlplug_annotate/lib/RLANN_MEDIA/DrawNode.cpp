...
RLANN_MEDIA_Node::readVideo(DS_TimePtr timeInfo, DS_BufferPtr destFrame, bool postUpdate)
{
    ...
    bufMetadata->insert("draw.frame", (qlonglong)timeInfo->videoFrame()); // drawing is on a session frame

    destFrame->appendAuxBuffer(annotationBuf);

    // have seen crashing here.. not sure why..
    if (_inputs[0] != nullptr)
    {
        _inputs[0]->readVideo(timeInfo, destFrame, postUpdate);
    } else
    {
        RLP_LOG_WARN("Invalid input node, cannot read")
    }
}
