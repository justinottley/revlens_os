...
RLP_SETUP_LOGGER(ext, RlpAnnBrush, Picker)

RlpAnnBrush_Picker::RlpAnnBrush_Picker():
...
RLANN_DS_BrushBase*
RlpAnnBrush_Picker::clone()
{
    return new RlpAnnBrush_Picker();
}


void
RlpAnnBrush_Picker::activate(DISP_GLView* view, RLANN_DS_DrawControllerBase* annCntrl)
{
    ...
    if (_view != view)
    {
        ...
        connect(
            ...
            this,
            &RlpAnnBrush_Picker::onDisplayZoomChanged
        );

        RLP_LOG_DEBUG("Connected to" << view)

        _dpr = view->scene()->view()->devicePixelRatio();
        _woffset = 4 / _dpr;
    }
...
void
RlpAnnBrush_Picker::setupFrame(RLANN_DS_AnnotationPtr ann, QVariantMap mdata)
{
    RLP_LOG_DEBUG("")
...
void
RlpAnnBrush_Picker::setImage(QImage img)
{
    // RLP_LOG_DEBUG(img.width() << img.height())

    _image = img.scaledToWidth(
        img.width() / _dpr,
...
void
RlpAnnBrush_Picker::onDisplayZoomChanged(float zoom)
{
    // RLP_LOG_DEBUG(zoom)
    if (_zoom != zoom)
    {
...
void
RlpAnnBrush_Picker::strokeMove(QPointF& point, QVariantMap mdata)
{
    // RLP_LOG_DEBUG(mdata)
    ...
    // RLP_LOG_DEBUG("Image:" << _image.width() << _image.height() << "View:" << (int)_view->width() << _view->height())

    if ((_image.width() + _woffset) != (int)_view->width())
    {
    ...
    QPointF displayPoint = mdata.value("event.pos").value<QPointF>();
...
void
RlpAnnBrush_Picker::onPlayStateChanged(int state)
{
...
void
RlpAnnBrush_Picker::paintGLPostDraw(GUI_Painter* painter, const QVariantMap& mdata)
{
    // RLP_LOG_DEBUG("")
    if (_playstate == 0) // playing
    {
