...
RLANN_GUI_AnnotationItem::onStrokeEnd()
{
    ...
    if (_annotation->historySize() > 1)
    {
        QImage annBg = _annotation->imageFromHistory(0);
        QImage* annImg = _annotation->getQImage();
        QImage flattenedImg = RLANN_DS_Annotation::flattenAnnotation(annBg, *annImg);

        _thumb = flattenedImg.scaledToHeight(40, Qt::SmoothTransformation);
        _thumb.rgbSwap();
    } else
    {
        RLP_LOG_WARN("history not found, thumb may have tile artifacts")
