        ...
        _height(70)
{
    ...
    GUI_SvgButton* upButton = new GUI_SvgButton(
        ":feather/arrow-up.svg", this, 20);
    
    upButton->setOutlined(true);
    upButton->icon()->setBgColour(QColor(200, 200, 200));
    upButton->setPos(33, 5);


    GUI_SvgButton* downButton = new GUI_SvgButton(
        ":feather/arrow-down.svg", this, 20);
    
    downButton->setOutlined(true);
        ...
        _col(QColor(200, 20, 20))
{
    ...
    setMenuXPosOffset(50);
    // setMenuYPosOffset(10);

    // Not available with QQuickItem?
    // _icon->setFlag(QQuickItem::ItemStacksBehindParent, false);

    // TODO FIXME: this is a really weird setup for a GUI item.
    // This is to be deprecated for the Brush Settings combined
    // widget
    _sizeMenuItem = new RLANN_GUI_BrushSizeMenuItem(_menu->itemWrapper());

    _menu->addItem(_sizeMenuItem);
    _menu->setWidth(_sizeMenuItem->width());
    _menu->setHeight(_sizeMenuItem->height());
    _menu->itemWrapper()->setWidth(_sizeMenuItem->width());
    _menu->itemWrapper()->setHeight(_sizeMenuItem->height());
