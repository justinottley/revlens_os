...
static void cb_mypaint_on_tile_request_end(MyPaintTiledSurface* tiled_surface, MyPaintTileRequest* request)
{
    ...
    if (tx > h->tilesWidth() || ty > h->tilesHeight() || tx < 0 || ty < 0)
    {
        qInfo() << "MP_Handler: cb_mypaint_on_tile_request_end(): NULL TILE";
    }
    else
    {
        RLANN_MP_Tile* tile = h->getOrInitTileFromIndex(QPoint(tx, ty));
        if (tile)
        {
            ...
            tile->updateImageFromTile(h->imgBuf());
        }
    }


    // qInfo() << "cb_mypaint_on_tile_request_end()" << tx << ty;
}


// ------------------------

RLANN_MP_Surface::RLANN_MP_Surface(QImage* imgBuf, QImage imgBackground):
    _imgBuf(imgBuf),
...
RLANN_MP_Surface::getOrInitTileFromIndex(const QPoint& idx)
{
    ...
    if (((int)idx.x() < k_max) && ((int)idx.y() < k_max))
    { // out of range ?
        ...
        if (!selectedTile)
        {
            // RLP_LOG_DEBUG("Creating new tile:" << idx.x() << idx.y())
            ...
            selectedTile->updateTileFromImage(&_imgBackground);
