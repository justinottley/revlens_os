...
RLANN_DS_Annotation::~RLANN_DS_Annotation()
{
    RLP_LOG_DEBUG("")
    _history.clear();
}
...
void
RLANN_DS_Annotation::clear()
{
    // RLP_LOG_DEBUG(_history.size())
    ...
    float relZoom)
{
    ...
    _windowMap.value(displayUuidStr)->update(width, height, yOffset, displayZoom, relZoom);
}
...
float
RLANN_DS_Annotation::sizeRatio(QString displayUuidStr)
{
    return _windowMap.value(displayUuidStr)->sizeRatio();
}
...
QImage
RLANN_DS_Annotation::extractAnnotation(QImage bg, QImage rawAnnotation)
{
    ...
    for (int y=0; y < height; ++y)
    {
        ...
        {
            QRgb bgPixel = bg.pixel(x, y);
            QRgb fgPixel = rawAnnotation.pixel(x, y);
...
QImage
RLANN_DS_Annotation::flattenAnnotation(QImage bg, QImage rawAnnotation)
{
...
void
RLANN_DS_AnnotationDisplayWindow::update(int width, int height, int yOffset, float displayZoom, float relZoom)
{
    // RLP_LOG_DEBUG(width << height << displayZoom << relZoom)

    _windowWidth = width;
    _windowHeight = height;
    _yOffset = yOffset;

    _zoom = displayZoom * (1.0 + relZoom);

    // _displayZoom = displayZoom; // relZoom; // displayZoom;
    // _relZoom = relZoom;
    
    _nDestWidth = _width * _zoom; //  + (1.0 * relZoom));
    _nDestHeight = _height * _zoom; //  + (1.0 * relZoom));
...
{
    float inX = inPoint.x();
    float inY = inPoint.y() - _yOffset;

    // float newZoom = _displayZoom * (1.0 + _relZoom);
    // float ndw = _width * newZoom;
    // float ndh = _height * newZoom;

    // float nl = ((float)_windowWidth - ndw) / 2.0;
    // float nt = ((float)_windowHeight - ndh) / 2.0;

    // float npX = (inX / newZoom) - (nl / newZoom);
    // float npY = (inY / newZoom) - (nt / newZoom);

    // QPointF np(npX, npY);

    // float scaleX = (float)_width / (float)_windowWidth;
    // float scaleY = (float)_height / (float)_windowHeight;
    ...
    // RLP_LOG_DEBUG("inpoint:" << inPoint)
    // RLP_LOG_DEBUG(scaleX << scaleY << "pan:" << panPercent << "displayZoom:" << _displayZoom << "relZoom:" << _relZoom)

    // QPointF p1(inX * scaleX, inY * scaleY);
    // return p1;
    ...
    QPointF p(
        (inX / _zoom) - (_nLeft / _zoom),
        (inY / _zoom) - (_nTop / _zoom)
    );


    // RLP_LOG_DEBUG(p << p1 << np);
    return p;
}


float
RLANN_DS_AnnotationDisplayWindow::sizeRatio()
{
    return _nDestWidth / _width;
}