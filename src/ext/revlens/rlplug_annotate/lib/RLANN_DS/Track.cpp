...
RLANN_DS_Track::RLANN_DS_Track(DS_Session* session, int idx, QUuid uuid, QString owner):
    DS_Track(session, idx, TRACK_TYPE_ANNOTATE, uuid, owner),
    _ghostFrameCount(0),
    _brush(nullptr)
{
    _annMap.clear();
...
void
RLANN_DS_Track::registerBrush(RLANN_DS_BrushBase* brush)
{
    RLP_LOG_DEBUG(brush->name())

    _brushMap.insert(brush->name(), brush);
}


void
RLANN_DS_Track::setBrush(QString brushName)
{
    if (!_brushMap.contains(brushName))
    {
        RLP_LOG_ERROR("Unknown brush type:" << brushName)
        return;
    }

    RLP_LOG_DEBUG(brushName)

    _brush = _brushMap.value(brushName);
}
