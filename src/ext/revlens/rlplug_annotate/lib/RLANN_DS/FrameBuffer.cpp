...
DS_BufferList
RLANN_DS_FrameBuffer::getBufferList()
{
    // RLP_LOG_DEBUG("")

    qlonglong frame = _bufferMetadata.value("draw.frame").value<long>();
    ...
    DS_ControllerBase* cntrl = _drawController->getMainController();
    ...
    for (int tx=0; tx != session->numTracks(); ++tx)
    {
        ...
        if ((tr->trackType() == TRACK_TYPE_ANNOTATE) && (tr->isEnabled()))
        {
    ...
    if (_drawController->isPaperTraceActive())
    {
        ...
        QVariantMap* md = ptfbuf->getBufferMetadata();

        if ((ptfbuf->getWidth() != _bufferMetadata.value("width").toInt()) || 
            (ptfbuf->getHeight() != _bufferMetadata.value("height").toInt()) ||
            (!md->contains("opacity")) ||
            (md->value("opacity").toFloat() != _drawController->paperTraceOpacity()))
        {
            // RLP_LOG_DEBUG("Reallocating Paper Trace Frame Buffer - opacity: " << _drawController->paperTraceOpacity())

            ptfbuf->reallocate(_bufferMetadata);
            ptfbuf->getQImage()->fill(Qt::white);
            md->insert("opacity", _drawController->paperTraceOpacity()); // 0.70);
        }
