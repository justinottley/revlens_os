...
void
RLFFMPEG_VideoReader::setFlushMode(bool flushModeVal)
{
    // RLP_LOG_DEBUG(flushModeVal)

    _flushMode = flushModeVal;
}
...
RLFFMPEG_VideoReader::openMedia()
{
    ...
    {
        RLP_LOG_WARN("Failed to initialize codec context")
        _sourceOk = false;
    }
...
RLFFMPEG_VideoReader::initializeCodecContext()
{
    ...
    if (avformat_open_input(&_fmtCtx, srcPath, NULL, &opts) < 0)
    {
        RLP_LOG_ERROR("Could not open source file: " << _srcFilePath)
        return -1; // TODO FIXME static const?
    }
    ...
    if (avformat_find_stream_info(_fmtCtx, NULL) < 0)
    {
        ...
        return -2; // TODO FIXME static const?
    }
...
RLFFMPEG_VideoReader::seekToFrame(qlonglong frameNum)
{
    ...
    if ((frameNum == 0) && (!_properties->contains("media.network")))
    {
...
RLFFMPEG_VideoReader::readNextPacket()
{
    ...
    if (!_sourceOk)
    {
        RLP_LOG_WARN("Source broken, aborting read")
    ...
    if (!p_ret.av_frame)
    {
        RLP_LOG_WARN("av_frame_alloc() alloc failed")
    ...
    if (_flushMode) 
    {
        ...
        if (ret_receive_frame != 0)
        {
            ...
            // RLP_LOG_DEBUG("flush mode - receive frame failed - end of file")
    ...
    while (!got_frame)
    {
        ...
        if (arf_ret >= 0) {
            ...
            if (my_pkt.stream_index == _streamIdx)
            {
                ...
                got_frame = true;
                p_ret.return_status = DS_Buffer::READ_OK;
        ...
        } else if (arf_ret == -541478725)
        {
            ...
            setFlushMode(true); // _flushMode = true;
...
RLFFMPEG_VideoReader::readVideo(DS_TimePtr timeInfo, DS_BufferPtr destBuffer, bool postUpdate)
{
    ...
    startCriticalReadSection();// _lock.lock();
    ...
    else
    {
        ...
        } else
        {
            ...
            if ((timeInfo->mediaVideoFrame() == 0) && (p_ret.return_status != DS_Buffer::READ_OK))
            {
                RLP_LOG_WARN("Got error reading start, trying again")
                p_ret = readNextPacket();
            }
    ...
    if (p_ret.return_status != DS_Buffer::READ_ERROR)
    {
    ...
    {
        RLP_LOG_WARN("error on packet read")
    }
    ...
    // TODO FIXME: HACK for streaming with annotations
    if (_properties->contains("extra.fbuf"))
    {
        DS_FrameBufferPtr fbuf = _properties->value("extra.fbuf").value<DS_FrameBufferPtr>();
        if (fbuf != nullptr)
        {
            destBuffer->appendAuxBuffer(fbuf);
        }
    }

    endCriticalReadSection(); // _lock.unlock();
