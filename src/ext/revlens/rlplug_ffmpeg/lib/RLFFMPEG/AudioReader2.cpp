    ...
    _audioStartVideoFrame(0)
{
    ...
    _outputBitDepth = ag->value("audio.output.bit_depth").toInt();
    _outputChannelCount = ag->value("audio.output.channel_count").toInt();
    _outputSampleRate = ag->value("audio.output.sample_rate").toInt();
...
RLFFMPEG_AudioReader::initializeScalars()
{
    if (!_sourceOk)
    {
        RLP_LOG_WARN("Source broken, aborting")
        return;
    }
...
RLFFMPEG_AudioReader::readAudio(DS_TimePtr timeInfo, DS_BufferPtr inBuffer)
{
    ...
    {
        RLP_LOG_WARN("Source broken, aborting read")
        inBuffer->getBufferMetadata()->insert("return_status", DS_Buffer::READ_ERROR);
        return;
    }
    ...
    if (p_ret.return_status == (int)DS_Buffer::READ_EOF)
    {
        ...
        } else
        {
            ...
            p_ret.return_status = DS_Buffer::READ_NONE;
        }
