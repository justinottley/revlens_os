    ...
    QVariantList* result)
{
    ...
    else
    {
        ...
        int outputSampleRate = mediaIn.value("audio.output.sample_rate").value<int>();
...
RLFFMPEG_Plugin::createAudioReader(QVariantMap* properties)
{
    ...
    RLFFMPEG_AudioReader* reader = new RLFFMPEG_AudioReader(properties);

    DS_NodePtr node(reader);

    if (reader->sourceOk())
    {
        AUDIO_ReadAllTask* readAllAudioTask = new AUDIO_ReadAllTask(node);
        getReadThreadPool(DS_Node::DATA_AUDIO)->start(readAllAudioTask);
    }
...
void
RLFFMPEG_Plugin::waitAndEvict()
{
    RLP_LOG_DEBUG("")

    QThreadPool* tp = getReadThreadPool(DS_Node::DATA_AUDIO);
