...
protected:
    ...
    virtual RLFFMPEG_PacketReadResult seekToFrame(qlonglong frameNum);

    virtual void setFlushMode(bool flushModeVal);

    virtual void startCriticalReadSection() { _lock.lock(); }
    virtual void endCriticalReadSection() { _lock.unlock(); }
