...
class RoomWidget(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent, room_name, room_uuid, room_media_list, created_at_str):
        ...
        self.action_button = RlpGui.GUI_SvgButton(
            ':feather/more-vertical.svg', self, 20)
        ...
        archive_action = action_menu.addItem(
            'Archive', {}, False
        )
        for action in [rename_action, delete_action, archive_action]:
        ...
        self.toggle_expand_button = RlpGui.GUI_SvgButton(
            ':feather/plus-circle.svg', self, 20
        )
        ...
        self.toggle_collapse_button = RlpGui.GUI_SvgButton(
            ':feather/minus-circle.svg', self, 20
        )
    ...
    def hoverLeaveEventItem(self, event):
        ...
        if not self.item_count_label.inHover() and not self.action_button.inHover():
            self.hideAll() 
    ...
    def on_action_requested(self, sig_info):
        ...
        if name == 'Delete':
            ...
            floating_pane = revlens.GUI.mainView().createFloatingPane(200, 200, False)
            diag = ConfirmDialog(
                floating_pane,
                'Delete Room',
            ...
            floating_pane = revlens.GUI.mainView().createFloatingPane(360, 220, False)
            diag = RenameDialog(
                'Rename Room',
            ...
            diag = ConfirmDialog(
                floating_pane,
                'Archive Room',
                'Archive Room "{}" - are you sure?'.format(info['room_name']),
                accept_label='Archive'
            )

            def _handle_archive_room(sig_info):
                self._parent.on_room_archive_requested(info['room_name'])

            diag.accept.connect(_handle_archive_room)
    ...
    def on_enter_requested(self, md):
        ...
        # Separate join and load messages - join results in a broadcast to the room,
        # load only returns room data to this client
        ...
        self._parent.enter_room_requested(self.room_uuid, self.room_name)
...
class RoomView(RlpGui.GUI_ItemBase):

    def __init__(self, parent):
        RlpGui.GUI_ItemBase.__init__(self, parent)

        self.rpane = RlpGui.GUI_Pane(self, 'room_view', 2)
        ...
        self.room_controls = RoomControlsWidget(self.rpane.panePtr(0))

        self.chatView = ChatPanel(self.rpane.panePtr(1))
...
class RoomsPanel(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent):
        ...
        self.show_remote_cursors_button = RlpGui.GUI_SvgButton(
            ':misc/cloud_cursor.svg', self, 20
        )
        self.show_remote_cursors_button.setToolTipText('Show Remote Cursors')
        ...
        self.start_review_button = RlpGui.GUI_SvgButton(
            ':misc/video.svg', self, 20)
        self.start_review_button.setToolTipText('Start a Review')
        ...
        self.toolbar_layout.addItem(self.new_room_button, 5)
        self.toolbar_layout.addItem(self.leave_button, 5)
        self.toolbar_layout.addDivider(23, 23, 3)
        self.toolbar_layout.addItem(self.start_review_button, 6)
        self.toolbar_layout.addDivider(23, 23, 3)
        self.toolbar_layout.addItem(self.show_remote_cursors_button, 6)
        self.toolbar_layout.addItem(self.status_text, 5)
        ...
        self.roomlist_sa.setPos(0, self.toolbar_layout.height() + 5)
        ...
        self.roomView = RoomView(self)
        self.roomView.setVisible(False)
        self.roomView.setPos(0, self.toolbar_layout.height() + 10)

        # self.room_controls.setPos(0, self.toolbar_layout.height() + 10)

        # self.layout.addItem(self.room_controls, 0)
        ...
        self.start_review_button.buttonPressed.connect(self.on_start_review_requested)

        self.room_cntrl.sendCallToClient.connect(self.on_data_received)

        revlens.CNTRL.noticeEvent.connect(self.on_notice_event)
    ...
    def onParentSizeChangedItem(self, width, height):
        ...
        self.roomView.setWidth(width)
        self.roomView.setHeight(nheight)
    ...
    def on_room_archive_requested(self, room_name):

        print('Archive Room: "{}"'.format(room_name))
    ...
    def on_new_room_clicked(self, md):
        ...
        floating_pane = revlens.GUI.mainView().createFloatingPane(400, 240, False)
    ...
    #     myIdent = self.room_cntrl.clientIdent()
    ...
    def on_start_review_requested(self, md):

        print('START REVIEW')
        revlens.CNTRL.emitNoticeEvent('rooms.start_review_requested', {})
    ...
    def enter_room_requested(self, room_uuid, room_name):

        self.roomlist_sa.setVisible(False)
        # self.room_controls.setVisible(True)
        self.roomView.setVisible(True)


        self.roomView.room_controls.load_room(room_uuid, room_name)
    ...
    def on_data_received(self, data):
        ...
        # print('on_data_received: {}'.format(pprint.pformat(data)))
        ...
        elif method_name in ['on_delete_room', 'on_rename_room', 'on_archive_room']:
            self._init_rooms()

        elif method_name == 'on_create_room' and self._inLoadWait:

            self.on_refresh_clicked()

            roomToJoin = self._inLoadWait
            self._inLoadWait = False

            room_info = data['kwargs']['result']
            if room_info['room_name'] == os.path.basename(roomToJoin):
                print('JOINGIN ROOM {} '.format(roomToJoin))
                ...
                self.enter_room_requested(room_info['room_uuid'], room_info['room_name'])
    ...
    def on_notice_event(self, evtName, evtInfo):

        # print('ROOMS NOTICE EVENT: {}'.format(evtName))

        if evtName == 'rooms.start_review_room':

            print('STARTIING REVIEW ROOM')
            print(evtInfo)

            media_type = evtInfo['media_type']
            roomName = os.path.basename(evtInfo['playlist_path'])
            if media_type == 'Reviewlist Media':
                
                self._inLoadWait = roomName
                revlens.CNTRL.emitNoticeEvent(
                    'media.request_start_review_with_playlist', evtInfo)

            elif media_type == 'Current Session':
                
                if not revlens.CNTRL.session().getTracksByType(10):
                    revlens.CNTRL.session().addTrackByType('Annotation')
    
                self.on_create_room_requested(
                    {'room_name': roomName, 'room_type': 'current_session'})

            elif media_type == 'Empty':
                self.on_create_room_requested(
                    {'room_name': roomName, 'room_type': 'empty'})


        elif evtName == 'load_complete' and self._inLoadWait:

            self.room_cntrl.callFromClient(
                'rooms.create_room',
                [],
                {
                    'room_name': self._inLoadWait,
