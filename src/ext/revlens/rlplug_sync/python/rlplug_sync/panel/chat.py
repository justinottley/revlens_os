...
class ChatPanel(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, parent):
        ...
        self.username = ag.get('user.username')
        self.user_info = {
            'first_name': ag.get('user.first_name'),
            'last_name': ag.get('user.last_name')
        }
        ...
        # self.room_label.setLeftSvgIconPath(':feather/home.svg', 20, 0)
    ...
    def add_message(self, room_name, room_uuid, text, username, user_info, create_time, emitSyncAction=True):
        ...
        self.message_list_layout.addItem(message, 10)
        self.message_list_layout.setHeight(self.message_list_layout.itemHeight())
    ...
    def on_data_received(self, payload):
        ...
        if action == 'message':
            ...
            self.room_label.setText('{} Chat'.format(room_name))
            self.room_label.setWidth(500)
            self.room_label.showItem()

            self.messages = []
            self.message_list_layout.clear()
