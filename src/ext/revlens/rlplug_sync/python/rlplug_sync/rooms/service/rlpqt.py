...
class RlpQtRoomServerController(object):
    ...
    def repo(self, site_name, project_name):
        return FilesysRoomsRepository.instance(site_name, project_name, self.EDBC)
    ...
    def _handle_delete_room(self, session_kwargs, *args, **kwargs):
        ...
        room_repo = self.repo(session_kwargs['site_name'], session_kwargs['project_name'])
        result = room_repo.delete_room(kwargs.get('room_name'))
    ...
    def _handle_archive_room(self, session_kwargs, *args, **kwargs):

        room_repo = self.repo(session_kwargs['site_name'], session_kwargs['project_name'])
        result = room_repo.archive_room(kwargs.get('room_name'))

        return self.build_return('ok', result, {'from_method': 'archive_room'})
    ...
    def _handle_create_room(self, session_kwargs, *args, **kwargs):
        ...
        result_dict = self.build_return('ok', new_room_info, {'from_method': 'create_room'})
    ...
    def _handle_update_room_session(self, session_kwargs, action, *args, **kwargs):
        ...
        self.LOG.debug('update_room_session(): {} "{}"'.format(session_kwargs, action))
    ...
    def _handle_join_room(self, session_kwargs, *args, **kwargs):
        ...
        # TODO FIXME: pull default room permissions
        ...
        # result = self.repo(session_kwargs.get('site_name')).add_client(room_name, client_tag)
        # if result:
        #     # self.room_name = room_name
        #     session_kwargs['room_name'] = room_name
        #     self.LOG.info('joined room: {}'.format(room_name))

        return self.broadcast('join_room', room_uuid, True, session_kwargs)
    ...
    def _handle_load_room(self, session_kwargs, *args, **kwargs):
        ...
        try:
            ...
            room_repo = self.repo(session_kwargs['site_name'], session_kwargs['project_name'])
            session_data = room_repo.get_room_session_data(room_name)
    ...
    def _handle_leave_room(self, session_kwargs, *args, **kwargs):
        ...
        if room_uuid and client_ident:
            ...
            room_repo = self.repo(session_kwargs['site_name'], session_kwargs['project_name'])
            result = room_repo.remove_client(
                room_name,
                session_kwargs.get('client.user')
            )

            # The websocket server is gonna broadcast a leave message to the rest of the room
            #
    ...
    def _handle_get_rooms(self, session_kwargs, *args, **kwargs):
        ...
        room_repo = self.repo(session_kwargs['site_name'], session_kwargs['project_name'])
        result = room_repo.get_rooms(**kwargs)
    ...
    def _handle_get_clients(self, session_kwargs, *args, **kwargs):
        ...
        client_info_list = revlens_prod.site.server.CNTRL.wsServer().getBroadcastGroupClientInfo(kwargs.get('room_uuid'))
        result_dict = self.build_return('ok', client_info_list, {'from_method': 'get_clients'})
