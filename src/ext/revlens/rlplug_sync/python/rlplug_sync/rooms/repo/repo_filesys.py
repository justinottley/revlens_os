...
class FilesysRoomsRepository(object):
    ...
    def __init__(self, site_name, project_name, edbc):
    ...
    @classmethod
    def instance(cls, site_name, project_name, edbc):
        if cls._INSTANCES.get(site_name) is None:
            cls._INSTANCES[site_name] = {}
            
        if cls._INSTANCES[site_name].get(project_name) is None:
            cls._INSTANCES[site_name][project_name] = FilesysRoomsRepository(site_name, project_name, edbc)

        return cls._INSTANCES[site_name][project_name]
    ...
    def _init_fs(self):
        ...
        self.LOG.info('_init_fs(): {} site_name: {} project_name: {}'.format(
            self.DIR_ROOT, self.site_name, self.project_name))

        self.LOG.info('Dir Root: {}'.format(self.dir_base))
    ...
    @property
    def dir_base(self):
        return os.path.join(self.DIR_ROOT, self.site_name, self.project_name)
    ...
    def _init_room(self, room_name, room_uuid=None, room_data=None, clean=False, session=None):
        ...
        if clean and os.path.isdir(room_dir):
            ...
            for root, dirs, files in os.walk(room_dir, topdown=False):
                ...
                if 'archive' in root:
                    # self.LOG.debug('Skipping: {}'.format(root))
                    continue
                ...
                for dir_entry in dirs:
                    ...
                    if 'archive' in dir_entry:
                        # self.LOG.debug('Skipping {}/{}'.format(root, dir_entry))
                        continue
    ...
    def get_room_session_data(self, room_name):
        ...
        self.LOG.debug('Got room data: {} {}'.format(room_name, room_uuid))
    ...
    def update_room(self, room_name, action, update_info):
        ...
        if action == 'addTrack':
        ...
        elif action in ['load_media_list', 'loadMediaList', 'load_previous_shot']:
            ...
            if sess_io:
                ...
                for media_uuid, media_track_info in track_info['media'].items():
    ...
    def archive_room(self, room_name):
        ...
        now_suffix = '{}_{}_{}__{}_{}'.format(now.year, now.month, now.day, now.hour, now.minute)

        room_dir = self._get_room_dir(room_name)
        archive_dir = os.path.join(room_dir, 'archive')
        if not os.path.isdir(archive_dir):
            self.LOG.info('Creating {}'.format(archive_dir))
            os.mkdir(archive_dir)

        archive_room_dir = os.path.join(archive_dir, now_suffix)
        if os.path.isdir(archive_room_dir):
            self.LOG.warning('Archive dir already exists: {}'.format(archive_room_dir))
            # return False

        else:
            self.LOG.info('Creating {}'.format(archive_room_dir))
            os.mkdir(archive_room_dir)
        ...
        for root, dirs, files in os.walk(room_dir):
            ...
            for file_entry in files:
                src_path = os.path.join(root, file_entry)
                src_suffix = root.replace(room_dir, '')[1:]
                dest_path = os.path.join(archive_room_dir, src_suffix, file_entry)
