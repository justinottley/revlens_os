    ...
    _doDataSync(false)
{
    ...
    connect(
        ag,
        &UTIL_AppGlobals::changed,
        this,
        &RLSYNC_RoomController::initSessionData
    );

    connect(
        ag,
        &UTIL_AppGlobals::updated,
        this,
        &RLSYNC_RoomController::initSessionData
    );
    ...
    // #ifdef SCAFFOLD_IOS
    // _skwargs.insert("project_name", "RLP");
    // _skwargs.insert("site_name", "rlp");
    // #endif
...
bool
RLSYNC_RoomController::setMainController(DS_ControllerBase* controller)
{
    ...
    mcntrl->addCommandFilter(_cmdFilter);
    ...
    connect(
        mcntrl->getMediaManager(),
        SIGNAL(mediaDataReady(QString, QVariantMap)),
        this,
        SLOT(onMediaDataReady(QString, QVariantMap))
    );
...
void
RLSYNC_RoomController::initSessionData(QVariantMap appGlobalsInfo)
{
    // LOG_Logging::pprint(appGlobalsInfo);
    ...
    // RLP_LOG_DEBUG(appGlobalsInfo)


    // QVariantMap* appGlobals = UTIL_AppGlobals::instance()->globalsPtr();

    // LOG_Logging::pprint(appGlobalsInfo);

    if (appGlobalsInfo.contains("project.name"))
    {
        QString projectName = appGlobalsInfo.value("project.name").toString();
        _skwargs.insert("project_name", projectName);
    }

    if (appGlobalsInfo.contains("edb.site_name"))
    {
        QString siteName = appGlobalsInfo.value("edb.site_name").toString();
        _skwargs.insert("site_name", siteName);
    }

    // RLP_LOG_INFO("Session Info:" << _skwargs)
    // LOG_Logging::pprint(_skwargs);
}
...
void
RLSYNC_RoomController::joinRoom(QString roomName, QString roomUuid, QVariantMap roomPermissions)
{
    ...
    RLP_LOG_DEBUG(_currentUserIdent << "joining room:" << roomName)
}
...
void
RLSYNC_RoomController::onLoginSuccessful()
{
    _currentUserIdent = QString("%1%2").arg(_currentUser).arg(clientIdent());
    _cmdFilter->setCategoryPermission(_currentUserIdent, "internal", true);

    RLP_LOG_DEBUG(_currentUserIdent)
}
...
void
RLSYNC_RoomController::onMediaRemoved(DS_NodePtr node, QString trackUuidStr, qlonglong atFrame, bool doSync)
{
    RLP_LOG_DEBUG("");
...
void
RLSYNC_RoomController::onMediaDataReady(QString dataType, QVariantMap info)
{
    return;
...
RLSYNC_RoomController::callFromClient(QString method, QVariantList args, QVariantMap kwargs)
{
    ...
    qobject_cast<CNTRL_MainController*>(_controller)
        ->getServerManager()
        ->siteClientSendCall(method, args, kwargs, _skwargs);
}
...
RLSYNC_RoomController::onRemoteCallReceived(QVariantMap msgEnvelope)
{
    ...
    // LOG_Logging::pprint(msgEnvelope);

    // TODO FIXME: No obvious way to tell that this message is a sync message intended to be processed here
    //
    ...
    if (fromMethod == "create_room")
    {
        ...
        kwargs.insert("result", msgObj.value("result"));
    ...
    } else if (fromMethod == "archive_room")
    {

        RLP_LOG_DEBUG("received archive_room, emitting" );
        ...
        kwargs.insert("result", msgObj.value("result").value<QVariantList>());
        msg.insert("kwargs", kwargs);
        msg.insert("method", "on_archive_room");
    ...
    } else if (fromMethod == "load_room")
    {
        ...
        QVariantMap roomPermissions = result.value("room_permissions").toMap();
        ...
        int numLoaded = qobject_cast<CNTRL_MainController*>(_controller)
            ->getMediaManager()
            ->loadSessionObject(sessionData, false);

        if (sync)
        {
            joinRoom(roomName, roomUuidStr, roomPermissions);
        }

        // CAN CRASH ON MACOS???
        // Doesn't make sense with async load anyway
    ...
    } else if ((fromMethod == "join_room") || (fromMethod == "client_leave"))
    {

        // somebody else joined the room
        emit sendCallToClient(msgObj);
...
RLSYNC_RoomController::handleClientCmd(QString method, QVariantMap kwargs, QString fromClient)
{
    ...
    } else if (method == "onScrubStart")
    {
        ...
        _controller->getAudioManager()->onScrubStart(frame);
    ...
    } else if (method == "gotoFrame")
    {
        ...
        qobject_cast<CNTRL_MainController*>(_controller)
            ->gotoFrame(frame, doRecenter, true, noEmitAction);
    ...
    } else if (method == "setOutFrame")
    {
        ...
        for (auto vit : GUI_TL2_Controller::instance()->views())
        {
            ...
            vit->update();
               ...
               (method == "drawEnd") ||
               (method == "hoverMove") ||
               (method == "setBrushSettings"))
    {

        DS_EventPlugin* evp = qobject_cast<CNTRL_MainController*>(_controller)
            ->getEventPluginByName("RlpAnnotation");
    ...
    else if (method == "playlist.changed")
    {
        _controller->emitNoticeEvent("playlist.on_changed", kwargs);
    }
    else if (method == "loadMediaList")
    {
        RLP_LOG_DEBUG("LOAD MEDIA LIST")
        ...
        if (track)
        {
            RLP_LOG_DEBUG("Got track for load:" << trackUuidStr)

            QVariantList mediaInputList = kwargs.value("media_input_list").toList();
            int trackIdx = track->index();
            qlonglong atFrame = kwargs.value("start_frame").toLongLong();
            bool doClearTrack = kwargs.value("clear").toBool();

            qobject_cast<CNTRL_MainController*>(_controller)->getMediaManager()->loadMediaList(
                mediaInputList,
                trackIdx,
                "",
                atFrame,
...
RLSYNC_RoomController::handleRoomUpdateCmd(QVariantList args, QVariantMap kwargs, QVariantMap sessionKwargs)
{
    ...
    } else if (method == "mediaRemoved")
    {
        RLP_LOG_DEBUG("MEDIA REMOVED")
        RLP_LOG_DEBUG(args);
        LOG_Logging::pprint(kwargs);

        CNTRL_MainController* cntrl = qobject_cast<CNTRL_MainController*>(_controller);
        QString trackUuid = kwargs.value("track_uuid").toString();
        DS_TrackPtr track = cntrl->session()->getTrackByUuid(trackUuid);
        qlonglong frame = kwargs.value("frame").toLongLong();

        if (track)
        {
            RLP_LOG_DEBUG("GOT TRACK AT" << trackUuid)
            track->removeNodeByFrame(frame);
        }
...
RLSYNC_RoomController::onSyncAction(QString action, QVariantMap actionKwargs)
{
    ...
    qobject_cast<CNTRL_MainController*>(_controller)
        ->getServerManager()
        ->siteClientSendCall("rooms.client_cmd", args, kwargs, _skwargs);
