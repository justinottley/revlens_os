...
public slots: // connections with the main controller
    ...
    void initSessionData(QVariantMap appGlobalsInfo);
    ...
    void onMediaDataReady(QString dataType, QVariantMap info);
...
public slots: // real-time sync
    ...
    QString currentUserIdent() { return _currentUserIdent; } // saved instead of rebuilt on every call for performance

    QVariantMap currentRoomPermissions() { return _roomPermissions; }

    QVariantMap skwargs() { return _skwargs; }

    void joinRoom(QString roomName, QString roomUuid, QVariantMap roomPermissions);
    void leaveRoom();
...
private:
    ...
    QString _currentUserIdent;

    QString _roomName;
    QString _roomUuid;
    QVariantMap _roomPermissions;
