...
class OCIOToolbarHandler(RlpGui.GUI_ItemBase):
    ...
    def __init__(self, viewerPane):

        dcontrols = viewerPane.getToolbarByName('Display Controls')
        RlpGui.GUI_ItemBase.__init__(self, dcontrols)
        ...
        # dcontrols.layout.addDivider(20, 25, 0)
        dcontrols.layout.addSpacer(10)
        dcontrols.layout.addItem(self.ocio_button, 10)

        self._view = viewerPane.view()
        self.na_label = RlpGui.GUI_Label(self._view, "OCIO Controls Not Available")
        self.na_label.setBgColor(QtGui.QColor(180, 40, 40))
        self.na_label.setPos(600, 32)
    ...
    def toggle_tools(self, md=None):
        ...
        if not self.paintEngine.isEnabled():
            if not self.na_label.isVisible():
...
def init_toolbar(tool_info):
    ...
    viewerPane = tool_info['tool']
    ocio_handler = OCIOToolbarHandler(viewerPane)
