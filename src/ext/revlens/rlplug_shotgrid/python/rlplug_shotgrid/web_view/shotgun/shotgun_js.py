...
try {
    new QWebChannel(qt.webChannelTransport, function(channel) {
        ...
        window.app_globals = channel.objects.app_globals;
...
init_main = function()
{
    if (document.body.getElementsByClassName('sg_main').length != 0)
    {
        console.log('[ revlens ] Shotgun Page detected, starting mutation observer');
    ...
    // Login flow automation Step 1
    //
    if (document.body.classList.contains('login_page'))
    {
        console.log('[ revlens ] Sign In button page detected');

        sign_in_button = document.body.getElementsByTagName('button')[0];
        console.log(sign_in_button);

        sign_in_button.click();
    }

    // Login flow automation Step 2
    //
    if (document.getElementById('new_user_signin_form') || document.getElementsByClassName('EMAIL_REQUIRED').length == 1)
    {

        window.app_globals.globals().then(function(ag) {
            console.log('[ revlens ] Username input page detected')

            email_input = document.getElementById('userName');
            console.log(email_input);

            email_input.value = ag['user.email'];

            next_button = document.getElementById('verify_user_btn');
            next_button.click();
        });
    }
}
