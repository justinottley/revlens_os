...
def startup():
    ...
    def _onServiceStartupReady(serviceInfo, *args):

        serviceName = serviceInfo
        if not args:
            serviceName = serviceInfo.get('service_name')

        if serviceName != 'iosched':
            return
    ...
    from revlens_prod.panel.session.track_media import TrackMediaWidget
    from revlens_prod.panel.library.review.main import ReviewMainWidget

    from rlplug_shotgrid.panel.session.track import ShotGridTrackMediaWidgetHandler
    from rlplug_shotgrid.panel.library.playlist import ShotgridPlaylistGroup
    from rlplug_shotgrid.prod_data.playlist import ShotGridPlaylistDataSource

    TrackMediaWidget.setHandler(ShotGridTrackMediaWidgetHandler)
    ReviewMainWidget.setMediaWidget(ShotgridPlaylistGroup)


    from revlens_prod.client_ws import SiteWsClient
    SiteWsClient.register_ioclient('sg')

    from revlens_prod.prod_data import ProdDataObjectFactory
    showDataObj = ProdDataObjectFactory.getShowData()
    showDataObj.setFormatter('code', rlplug_shotgrid.cbFormatMediaData)
    showDataObj.setDataSource('playlist', ShotGridPlaylistDataSource())
