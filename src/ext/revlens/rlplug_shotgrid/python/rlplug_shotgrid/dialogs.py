...
class ShotgridLoadDialog(RlpGui.GUI_ItemBase):
    ...
    @classmethod
    def create(cls, media_title, load_path, thumb_path):

        if cls._INSTANCE:
            diag = cls._INSTANCE
            diag.setup(media_title, load_path, thumb_path)
            return diag

        fpane = revlens.GUI.mainView().createFloatingPane(350, 400, False)
        diag = cls(fpane, media_title, load_path, thumb_path)
        diag._INSTANCE = diag

        return diag

    def setup(self, media_title, load_path, thumb_path):
