...
def request_load_version(version_list, **kwargs):
    ...
    ver_info = sgh.find('Version',
        [['id', 'in', version_list]],
        ['code', 'image', 'sg_path_to_frames', 'frame_count', 'sg_department']
    )
    ver_info = ver_info[0]
    ...
    diag = ShotgridLoadDialog.create(
        ver_info['code'],
        ver_info,
        temp_thumb_path
    )
...
def request_load_playlist(review_list_entity, review_list_name=None, media_callback=None, **kwargs):
    print('LOAD PLAYLIST')
...
def request_load(entity_type, entity_info, **kwargs):
    ...
    if entity_type == 'Version':
        ...
        request_load_playlist(result[0], **kwargs)
