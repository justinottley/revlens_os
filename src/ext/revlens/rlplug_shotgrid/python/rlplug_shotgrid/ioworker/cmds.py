...
def get_sequence_media(project_id, sequence_id, media_callback=None, **kwargs):
    ...
    project_entity = {'type': 'Project', 'id': project_id} # app_globals['project.info']['id']}
    seq_entity = {'type': 'Sequence', 'id': sequence_id} # sequence_info['id']}
    ...
    for shot_entry in shot_result:
        ...
        ver_result = SG.find('Version',
            ...
            ['sg_path_to_movie', 'sg_path_to_frames', 'sg_department', 'sg_final',
             'code', 'user', 'image', 'frame_count', 'sg_testid'],
        ...
        if ver_result:
            ...
            # homogenize metadata entries

            # TODO FIXME: SWITCH TO FORMATTER
            codeIn = mediaMd['code']
            codeInParts = codeIn.split(':')
            if len(codeInParts) == 5:
                shotCode = codeInParts[2]
                shotCodeParts = shotCode.split('/')
                if len(shotCodeParts) == 2:
                    seqCode = shotCodeParts[0]
                    mediaMd['seq'] = seqCode

                mediaMd['code'] = shotCode
                mediaName = shotCode



            mediaMd['dept'] = mediaMd.get('sg_department')
            mediaMd['version'] = mediaMd.get('sg_testid')
            mediaMd['testid'] = mediaMd.get('sg_testid')
