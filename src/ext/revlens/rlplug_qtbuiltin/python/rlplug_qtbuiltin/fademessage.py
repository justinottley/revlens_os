...
def onNoticeEvent(noticeName, noticeInfo):
    ...
    if (noticeName == 'permissions_changed'):
        ...
        msg = '{} - {} {}'.format(noticeInfo['user_ident'], noticeInfo['category'], stateStr)
        rlplug_qtbuiltin.RLQTDISP_FADEMESSAGEPLUGIN_OBJ.displayMessage(
            msg, 1400, '')

    if (noticeName == 'load_complete'):
        msg = rlplug_qtbuiltin.RLQTDISP_FADEMESSAGEPLUGIN_OBJ.displayMessage(
            'Load Complete', 1400, '')
