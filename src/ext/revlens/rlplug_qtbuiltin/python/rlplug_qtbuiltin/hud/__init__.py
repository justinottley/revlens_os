...
def _get_sftset_dir():
    ...
    if 'ipad' in buildopt.get_platform().lower():
        sftset_dir = os.path.join(os.path.dirname(sftset_dir), 'static', 'revlens')

    return sftset_dir


def updateMetadataHud(props):
    ...
    for prop_name in prop_keys:
        ...
        elif prop_name == 'media.thumbnail.data':
            prop_val = '<base64 data>'

        elif prop_name in ['audio.waveform.path']:
            prop_val = '<skipped>'
    ...
    hud_cntrl.addHudConfig(
        'metadata',
        {
            ...
            'callback.node': 'rlplug_qtbuiltin.hud.updateMetadataHud'
        }
    )
...
def toggleMediaHud():
    ...
    if n:
        dispInfo = revlens.CNTRL.getDisplayMetadata()
        dispUuid = dispInfo['disp.0.display.uuid']
        curr_visible = hud_cntrl.isHudVisible(dispUuid, 'metadata')
        if curr_visible:
            hud_cntrl.setHudVisible(dispUuid, 'metadata', False)
            return

        updateMetadataHud(n.getProperties())

        hud_cntrl.setHudVisible(dispUuid, 'metadata', True)

        # pprint.pprint(props)
        # pprint.pprint(hud_list)

def setHudVisible(hudName, value):

    hud_cntrl = rlplug_qtbuiltin.RLQTDISP_HUD_OBJ
    dispInfo = revlens.CNTRL.getDisplayMetadata()

    mainDispKey = 'disp.0.display.uuid'
    if mainDispKey in dispInfo:
        dispUuid = dispInfo[mainDispKey]
        hud_cntrl.setHudVisible(dispUuid, hudName, value)
...
def startup():
    ...
    platform = ag.get('app.platform')

    for revlens_path_entry in os.getenv('REVLENS_PATH', '').split(':'):
        hud_config_dir = os.path.join(revlens_path_entry, 'etc', 'hud')

        if platform == 'ios':
            hud_config_dir = os.path.join(os.path.dirname(revlens_path_entry), 'static', 'revlens', 'etc', 'hud')

        LOG.debug('Checking for HUDs at {}'.format(hud_config_dir))

        if os.path.isdir(hud_config_dir):
