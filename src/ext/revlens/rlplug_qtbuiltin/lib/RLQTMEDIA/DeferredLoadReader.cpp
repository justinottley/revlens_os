    ...
    _isLoading(false)
{
    ...
    connect(
        this,
        &RLQTMEDIA_DeferredLoadReader::requestNamedClientCall,
...
void
RLQTMEDIA_DeferredLoadReader::loadDeferred(DS_Node::NodeDataType dataType)
{
    // RLP_LOG_DEBUG((int)_dataType)

    if (dataType == DS_Node::DATA_UNKNOWN)
    {
        dataType = _dataType;
    }
    ...
    QRunnable* task = new RLQTMEDIA_DeferredLoadTask(this, srcInfo, dataType);
...
RLQTMEDIA_DeferredLoadReader::loadMedia(QVariantMap srcInfo)
{
    ...
    if (_isLoading)
    {
        RLP_LOG_ERROR("Load in progress, aborting")
        return false;
    }
    ...
    QTimer::singleShot(1, this, [this]() {
        this->loadDeferred(DS_Node::DATA_ALL);
    });
...
RLQTMEDIA_DeferredLoadReader::setVideoInputNode(DS_NodePtr node)
{
    ...
    // Transfer flags
    //
    node->graph()->setFlags(graph()->flags());
    ...
    _properties->insert("media.metadata", props->value("media.metadata"));
    /*
...
RLQTMEDIA_DeferredLoadReader::readVideo(DS_TimePtr timeInfo, DS_BufferPtr destFrame, bool postUpdate)
{
    ...
    if (_videoNode == nullptr)
    {
        ...
        fbuf->getBufferMetadata()->insert("image.loading", true);
        ...
        p.drawText(100, (img->height() / 2) + yOffset, "Loading..");
