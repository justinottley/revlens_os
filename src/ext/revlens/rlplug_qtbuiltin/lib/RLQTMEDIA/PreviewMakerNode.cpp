...
DS_FrameBufferPtr
RLQTMEDIA_PreviewMakerNode::_readVideo(qlonglong mediaFrameNum)
{
    ...
    qlonglong sessionFrame = _inputs[0]->getProperty<long>("session.frame");

    RLP_LOG_DEBUG(mediaFrameNum << "session frame:" << sessionFrame)
    ...
    DS_TimePtr ftime = _inputs[0]->makeTime(mediaFrameNum + sessionFrame);
...
QImage
RLQTMEDIA_PreviewMakerNode::makePreview(qlonglong mediaFrameNum)
{
    RLP_LOG_DEBUG(mediaFrameNum << _inputs.size())

    DS_FrameBufferPtr fbuf = _readVideo(mediaFrameNum);
    ...
    QImage image = fbuf->toQImage();

    if (image.width() > 0)
    {
        emit imageReady(mediaFrameNum, image);
...
QVariantList
RLQTMEDIA_PreviewMakerNode::makePreviewAllBuffers(qlonglong frameNum)
{
    RLP_LOG_DEBUG(frameNum)

    DS_FrameBufferPtr fbuf = _readVideo(frameNum);

    QVariantList result;
    DS_FrameBuffer::exportFramebufferList(fbuf.get(), &result);
