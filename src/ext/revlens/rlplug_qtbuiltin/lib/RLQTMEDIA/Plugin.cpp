...
RLQTMEDIA_Plugin::identifyMedia(QVariantMap mediaInput, DS_Node::NodeDataType dataType)
{
    ...
    {
        // RLP_LOG_WARN("resolved_component not found")
        // return result;
    }
    ...
    {
        // RLP_LOG_WARN("frame component not found")
        // return result;
    }
    ...
    #ifdef __MACH__
    if (!(videoPath.endsWith(".png") || videoPath.endsWith(".jpg") || videoPath.endsWith(".tif") || videoPath.endsWith(".tiff")))
    #else
    if (!(videoPath.endsWith(".png") || videoPath.endsWith(".jpg")))
    #endif
    {
        // RLP_LOG_DEBUG("extension check failed, skipping")
        return result;
    }

    // RLP_LOG_DEBUG("Attempting identify:" << videoPath)
    ...
    if (numFrames > 0)
    {
        ...
        if (fmt != QImage::Format_Invalid)
        {
            ...
            {
                RLP_LOG_WARN(filePath << "- Converting to 8 bit / 4 channel")
        ...
        {
            RLP_LOG_WARN("Invalid media with correct extension")
        }
