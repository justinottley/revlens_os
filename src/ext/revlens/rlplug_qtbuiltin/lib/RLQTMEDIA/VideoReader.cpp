...
RLQTMEDIA_VideoReader::readVideo(DS_TimePtr timeInfo, DS_BufferPtr destFrame, bool postUpdate)
{
    ...
    // TODO FIXME HACK
    DS_FrameBuffer::ColourSpace cspace = DS_FrameBuffer::COLSPACE_NONLINEAR;

    if (img.format() != QImage::Format_RGBA8888)
    {
        RLP_LOG_WARN(frameNum << "- Converting to 8 bit / 4 channel")
        img = img.convertToFormat(QImage::Format_RGBA8888).rgbSwapped();
    }
    ...
    fbuf->setColourSpace(cspace); // DS_FrameBuffer::COLSPACE_NONLINEAR
