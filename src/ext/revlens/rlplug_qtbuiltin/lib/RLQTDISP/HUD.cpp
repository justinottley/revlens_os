...
RLQTDISP_HUD::RLQTDISP_HUD() :
    ...
    _lock(new QMutex()),
    _inPress(false),
    _lastNodeUuid("") // TODO FIXME REMOVE
{
    _hudConfig.clear();
}
...
bool
RLQTDISP_HUD::setMainController(DS_ControllerBase* controller)
{
...
RLQTDISP_HUD::reset()
{
    if (_childPlugins.size() > 0) // for multiple display support
    {
        ...
        {
            RLQTDISP_HUD* chud = qobject_cast<RLQTDISP_HUD*>(_childPlugins.at(i));
            if (chud->display() != nullptr)
            {
                chud->reset();
            }
        }
...
bool
RLQTDISP_HUD::hasHud(QString displayUuid, QString hudName)
{
    if (_childPlugins.size() > 0) // for multiple display support
    {
        for (int i=0; i != _childPlugins.size(); ++i)
        {
            RLQTDISP_HUD* chud = qobject_cast<RLQTDISP_HUD*>(_childPlugins.at(i));
            if ((chud->display() != nullptr) && (chud->display()->uuid() == displayUuid))
            {
                return chud->hasHud(displayUuid, hudName);
            }
        }

    }

    return _hudConfig.contains(hudName);
}


void
RLQTDISP_HUD::setHudVisible(QString displayUuid, QString hudName, bool isVisible)
{
    ...
    if (_childPlugins.size() > 0) // for multiple display support
    {
        for (int i=0; i != _childPlugins.size(); ++i)
        {
            ...
            if ((chud->display() != nullptr) && (chud->display()->uuid() == displayUuid))
            {
                chud->setHudVisible(displayUuid, hudName, isVisible);
            }
        }
    ...
    if (_display == nullptr)
    {
        RLP_LOG_WARN("Invalid display")
        return;
    }

    if (_display->uuid() != displayUuid)
    {
        return;
    }
    ...
    // RLP_LOG_DEBUG(_display->uuid() << hudName << isVisible)
    ...
    if (_controller->getPlaybackState() == CNTRL_MainController::PLAYSTATE_PAUSED)
    {
        _display->update();
    }
}
...
bool
RLQTDISP_HUD::isHudVisible(QString displayUuid, QString hudName)
{
    if (_childPlugins.size() > 0) // for multiple display support
    {
        RLP_LOG_DEBUG("Querying children for" << displayUuid << hudName)
        for (int i=0; i != _childPlugins.size(); ++i)
        {
            RLQTDISP_HUD* chud = qobject_cast<RLQTDISP_HUD*>(_childPlugins.at(i));
            if ((chud->display() != nullptr) && (chud->display()->uuid() == displayUuid))
            {
                RLP_LOG_DEBUG("CHILD OK")
                return chud->isHudVisible(displayUuid, hudName);
            }
        }
    }
    ...
    RLP_LOG_DEBUG(displayUuid << hudName)
    ...
    {
        RLP_LOG_WARN("HUD not found:" << hudName)
    }
...
RLQTDISP_HUD::paintGLPostDraw(GUI_Painter* painter, const QVariantMap& metadata)
{
    ...
    QVariantList cbArgList;

    // RLP_LOG_DEBUG("")

    // QMutexLocker l(_lock); // TODO FIXME: is this necessary?
    ...
    if (currNode == nullptr)
    {
        // RLP_LOG_DEBUG("Abort, no current node")
        return;
    }

    // If we're in a composite, see which buffers are enabled

    QVariantMap* nprops = currNode->getPropertiesPtr();
    DS_Node* compositeNode = nprops->value("graph.node.composite").value<DS_Node*>();
    if (compositeNode != nullptr)
    {
        

        int compInputs = compositeNode->inputCount();
        if (compInputs > 1)
        {

            int selIdx = 1;
            int dIdx = metadata.value("display.idx").toInt();
            if (dIdx == 2)
            {
                selIdx = 0;
            } else
            {
                ...
                if (metadata.value("node.composite.2.enabled").toBool())
                {
                    selIdx = 2;
                }
            }

            // RLP_LOG_DEBUG("IN A COMPOSITE")
            // LOG_Logging::pprint(metadata);

            // QMap<int, bool> enabledMap;
            
            // int lastEnabledInput = 0;
            // for (int i=0; i!=compInputs; ++i)
            // {
            //     QString enabledKey = QString("node.composite.%1.enabled").arg(i);
            //     if ((metadata.contains(enabledKey)) && (metadata.value(enabledKey).toBool()))
            //     {
            //         // RLP_LOG_DEBUG("Found key:" << enabledKey)
            //         lastEnabledInput = i;
            //         enabledMap.insert(i, true);
            //     }
            // }

            // if (enabledMap.size() == 1)
            // {
            //     // One enabled input, pick it
            //     // RLP_LOG_DEBUG("Switching to input:" << lastEnabledInput)
            //     currNode = compositeNode->input(lastEnabledInput);
            // }

            currNode = compositeNode->input(selIdx);
        }
    }

    // TODO FIXME: HACK for metadata hud update
    if ((currNode != nullptr) && (_lastNodeUuid == ""))
    {
        _lastNodeUuid = currNode->graph()->uuid().toString();
    }
    ...
    {
        QString hudName = *hudNameIt;

        QVariantMap hudInfoData = _hudConfig.value(hudName).toMap();
        ...
        if (hudInfoData.contains("callback.node") && (currNode != nullptr))
        {
            QString thisNodeUuid = currNode->graph()->uuid().toString();
            if (_forceReload || (_lastNodeUuid != thisNodeUuid) || (currHud.size() == 0))
            {
                QString hudNodeUpdateCallback = hudInfoData.value("callback.node").toString();

                RLP_LOG_DEBUG("Running callback:" << hudNodeUpdateCallback)
                PY_Interp::call(hudNodeUpdateCallback, cbArgList);

                // Pull potentially updated metadata again
                hudInfoData = _hudConfig.value(hudName).toMap();
                currHud = hudInfoData.value("item_list").value<QVariantList>();
                _lastNodeUuid = thisNodeUuid;
            }
