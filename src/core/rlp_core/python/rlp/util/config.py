...
def _init_config_dir():
    ...
    if os.getenv('RLP_APP_PLATFORM') == 'ios':
        rlp_root_dir = os.path.join(os.getenv('TMPDIR'), '.config', 'rlp')
    else:
        rlp_root_dir =  os.path.expanduser('~/.config/rlp')

    if not os.path.isdir(rlp_root_dir):
        LOG.debug('Creating {}'.format(rlp_root_dir))
        os.makedirs(rlp_root_dir)

    for subdir in ['webengine_cache']:
        sub_full_dir = os.path.join(rlp_root_dir, subdir)
        if not os.path.isdir(sub_full_dir):
            print('Creating {}'.format(sub_full_dir))
            os.mkdir(sub_full_dir)
