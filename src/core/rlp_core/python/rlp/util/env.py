...
def mprint(msg):
    if os.getenv('RLP_SHOW_LOG'):
        print(msg)


mprint('--')
mprint('Inst Dir: {idir}'.format(idir=INST_DIR))
mprint('Run Top Dir: {rdir}'.format(rdir=RUN_TOP_DIR))
mprint('--')

os.environ['RLP_INST_DIR'] = INST_DIR
if not os.getenv('RLP_INST_BIN_DIR'):
    os.environ['RLP_INST_BIN_DIR'] = os.path.join(INST_DIR, 'core/bin')
...
def setup_env_ffmpeg(env, tb_dir):
    ...
    ff_lib_dir = os.path.join(tb_dir, 'FFmpeg', ffmpeg_version, lib_dir)
    ff_bin_dir = os.path.join(tb_dir, 'FFmpeg', ffmpeg_version, bin_dir)
        ...
        for project in [
            ...
            for lib_dir in ['lib64', 'lib']:
                ...
                if os.path.isdir(proj_lib_dir):
                    ...
                    # mprint('NOT FOUND: {}'.format(proj_lib_dir))
                    pass


    # Always use built-in OCIO config for now
    if True: # not os.getenv('OCIO'):
        mprint('WARNING: OCIO env. variable not set, using built-in default')
        ...
        config = 'studio-config-v2.2.0_aces-v1.3_ocio-v2.4.ocio'

        if os.name == 'nt':
            config = 'cg-config-v1.0.0_aces-v1.3_ocio-v2.1.ocio'

        ocio_config_path = os.path.join(tb_dir, 'OpenColorIO', ocio_version, 'config', config)
        os.environ['OCIO'] = ocio_config_path.replace('\\', '/')


def setup_env_otio(env, tb_dir):

    otio_version = '25_10_py311'
    otio_dir = os.path.join(tb_dir, 'OpenTimelineIO', otio_version)
    otio_lib_dir = os.path.join(otio_dir, 'lib')
    otio_py_dir = os.path.join(otio_lib_dir, PY_DIR, 'site-packages')

    env[LD_LIB_PATH_KEY].insert(0, otio_lib_dir)
    env['PYTHONPATH'].append(otio_py_dir)
...
def setup_env_oiio(env, tb_dir):
    ...
    project_list = linux_project_list
    if platform.system() == 'Darwin':
        project_list = darwin_project_list
...
def setup_env_mypaint(env, tb_dir):
    ...
    mypaint_lib_dir = os.path.join(tb_dir, 'qtmypaint', '6.8.0', platstr, 'lib')
...
def setup_egg_pylibs(env, pydir):
    ...
    for pth_entry in pth_entries:
        ...
        mprint('Reading .pth: {}'.format(pth_path))
...
def setup_env_scaffold(env, tb_dir):
    ...
    os.environ['SC_THIRDBASE_VERSION'] = '25_04'
...
def setup_env_thirdparty(env, tb_dir):
    ...
    # macOS app bundle..
    envPylibTbDir = os.getenv('RLP_TB_PYLIB_DIR')
    if envPylibTbDir:
        tb_dir = envPylibTbDir
        print('Using Thirdbase pylib dir: {}'.format(tb_dir))
    ...
    # TODO FIXME: build config for extra pylibs?
    # add shipped libs...

    pylibDir = os.path.join(tb_dir, 'pylibs')
    if os.path.isdir(pylibDir):
        for dirEntry in os.listdir(pylibDir):
            pylibVer = '0.0' # TODO FIXME HARDCODED FOR NOW
            pylibVerDir = os.path.join(pylibDir, dirEntry, pylibVer)
            if os.path.isdir(pylibVerDir):
                print('Adding pylib: {} {}'.format(dirEntry, pylibVer))
                lib_list.append(
                    ('pylibs/{}'.format(dirEntry), pylibVer)
                )
    ...
    for lib_entry in lib_list:
        ...
        if os.path.isdir(pylib_dir):
            ...
            mprint('WARNING: skipping {}'.format(pylib_dir))
...
def setup_env_qt(env, tb_path):
    ...
    if platform.system() == 'Linux':
        ...
        if 'CentOS' in linux_dist_name:
            ...
            mprint('LD_PRELOAD: {n} libraries'.format(n=len(preload_list)))
    ...
    # mprint(os.environ['QT_PLUGIN_PATH'])
    # mprint(os.environ['QTWEBENGINEPROCESS_PATH'])
...
def setup_env_extrevlens(env, ext_name):
    ...
    if not os.getenv('REVLENS_PATH'):
        mprint('WARNING: REVLENS_PATH not found')
    ...
    if os.path.isdir(extrevlens_dir):
        ...
        extrevlens_bin_dir = os.path.join(extrevlens_dir, 'bin')
        ...
        env['PATH'].insert(0, extrevlens_bin_dir)
        ...
        if os.path.isdir(extrevlens_py_dir):
            ...
            mprint('WARNING: extrevlens python plugin dir not found: {}'.format(extrevlens_py_dir))
...
def setup_env(update_sys_path=False, full=True):
    ...
    sc_platform = buildopt.get_platform()
    ...
    if os.getenv('SC_ENV_INIT'):
        ...
        if sc_tb_dir and os.path.isdir(sc_tb_dir):
            tb_dir = sc_tb_dir
            mprint('Got Thirdbase Dir from URI: {}'.format(tb_dir))

    if not tb_dir:
        tb_dir = os.path.join(RUN_TOP_DIR, 'thirdbase', THIRDBASE_VERSION, sc_platform) #, 'scaffoldscaffold.vc.get_software_set('thirdbase').path
        if not os.path.isdir(tb_dir):
            mprint('Thirdbase dir not found, attempting parent: {}'.format(tb_dir))
            tb_dir = os.path.join(os.path.dirname(RUN_TOP_DIR), 'thirdbase', THIRDBASE_VERSION, sc_platform)
            if not os.path.isdir(tb_dir):
                mprint('Thirdbase dir not found, attempting parent: {}'.format(tb_dir))
                tb_dir = os.path.join(os.path.dirname(os.path.dirname(RUN_TOP_DIR)), 'thirdbase', THIRDBASE_VERSION, sc_platform)

    mprint('Thirdbase: {}'.format(tb_dir))
    mprint('')
    ...
    # Flatten env
    if ldlib_path_key != 'PATH':
        privkey = '_{}'.format(ldlib_path_key)
        existing_lib_path = os.getenv(privkey, '')
        os.environ[ldlib_path_key] = os.pathsep.join(env[ldlib_path_key]) + os.pathsep + existing_lib_path
    ...
    mprint('')
    mprint('PYTHONHOME')
    mprint('    {}'.format(os.getenv('PYTHONHOME')))
    mprint('')
    mprint('')
    ...
    if full:
        ...
        mprint('')
        mprint('OCIO')
        mprint('    {}'.format(os.getenv('OCIO')))
        mprint('')

    if update_sys_path:
        sys.path = env['PYTHONPATH'] + sys.path
        
        if platform.system() == 'Darwin': # This should be running from a framework or from a bundle
            badSysPath = '/Library/Python/Versions/{}/lib/python{}/site-packages'.format(
                PY_VERSION, PY_VERSION
            )
            if badSysPath in sys.path:
                sys.path.remove(badSysPath)
