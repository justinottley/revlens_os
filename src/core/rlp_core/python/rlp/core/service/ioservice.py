...
class MessageReceiver(JsonRPCClient):
    ...
    def __init__(self, ws_url, name='iosched', *args, **kwargs):
        JsonRPCClient.__init__(self, ws_url, *args, **kwargs)
    ...
    def _init_service_mode(self):
        ...
        self.send(self.build_message('register_service', [self.name], {}))
        # self._wait()
...
def main():
    ...
    io_log_handler = JsonRPCLogHandler(ws_url, encrypted=False)
    ...
    mr = MessageReceiver(ws_url, name='iosched', encrypted=False)
    mr.connect()
