...
def _init_plugins(env_var_name='REVLENS_PATH', prefix_keys=None, startup_filename='startup.py'):
    ...
    for entry in os.getenv(env_var_name).split(os.pathsep):
        ...
        for dir_entry in sorted(os.listdir(python_path)):
            ...
            if os.path.isdir(pydir):
                ...
                plugin_startup_path = os.path.join(pydir, startup_filename)
...
def _init():
    ...
    try:
        ...
        if notify_error:
            print('Unable to setup AUTH object {}'.format(e))
