...
class RlpLogger(logging.Logger):
    ...
    def progress(self, run_id, message, payload=None):
        payload = payload or {}
        payload['run_id'] = run_id
        return self.log(logging.PROGRESS, message, extra={'extra': payload})

    def result(self, status, run_id, result_dict):
        return self.log(logging.RESULT, '', extra={'extra': {
            'status': status,
            'run_id': run_id,
            'result':result_dict}})
