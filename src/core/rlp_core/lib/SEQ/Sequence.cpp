...
RLP_SETUP_LOGGER(core, SEQ, Sequence)
...
SEQ_Sequence::match(SEQ_ItemPtr item)
{
    ...
    if (_itemMap.size() == 0)
    {
        ...
        for (it = _hintList.begin(); it != _hintList.end(); ++it)
        {
            ...
            // RLP_LOG_DEBUG("Attempting match with hint " << (*it)->name());
            ...
            if (res.idx != -1)
            {
                ...
                // RLP_LOG_DEBUG("MATCHED with:" << (*it)->name())
    ...
    { 
        ...
        // RLP_LOG_DEBUG("checking signature - iter idx:" << iterIdx)
