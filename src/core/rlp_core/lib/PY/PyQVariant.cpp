...
PyQVariant::PyQVariant(py::handle pyobj)
{
    _val.setValue(pyobj);
}
...
PyQVariant::pyval()
{
    ...
    } else if ((valTypeName == "qlonglong") || (valTypeName == "qulonglong"))
    {
    ...
    else if (valTypeName == "QScreen*") // TODO FIXME: register converters instead of hardcoding these
    {
        pval = py::cast(_val.value<QScreen*>());
    }
    else if (valTypeName == "QPrinter*") // TODO FIXME: register converter
    {
        #ifndef SCAFFOLD_IOS
        pval = py::cast(_val.value<QPrinter*>());
        #else
        qInfo() << "QPrinter not available";
        #endif
    }
    ...
    } else if (valTypeName == "pybind11::handle")
    {
        return py::reinterpret_borrow<py::object>(_val.value<py::handle>());
...
PyQVariant::fromPyHandle(py::handle objIn)
{
    ...
    else
    {
        ...
        } else if (pyClass == "QUuid")
        {
            val.setValue(objIn.cast<QUuid>());
