...
QString
CoreNet_WebSocketClient::formatMessage(QVariantMap msgObj)
{
    QJsonObject result = QJsonObject::fromVariantMap(msgObj);
    QString plainTextMsg = QString(QJsonDocument(result).toJson(QJsonDocument::Compact));
...
void
CoreNet_WebSocketClient::sendCall(
    QString method,
    QVariantList args,
    QVariantMap kwargs,
    QVariantMap skwargs,
...
)
{
    ...
    if (callback != nullptr)
    {
        skwargs.insert("run_id", callback->runId());
        _callbackMap.insert(callback->runId(), callback);
    }
...
{
    // RLP_LOG_DEBUG(_crypt->decryptToString(msg))
...
CoreNet_WebSocketClient::onTextMessageReceived(QString rawMsg)
{
    ...
    if (_callbackMap.contains(runId))
    {
        CoreNet_RPCFuture* callback = _callbackMap.value(runId);
        if (callback != nullptr) // sanity...
        {
            callback->onResultReady(msgObj);
            _callbackMap.remove(runId);
        }
    } else
    {
...
{
    QString errMsg = _wsocket.errorString();

    RLP_LOG_WARN(errMsg);

    _connected = false;

    emit clientDisconnected(_url.toString(), errMsg);
...
CoreNet_WebSocketClient::reconnect()
{
    ...
    {
        RLP_LOG_WARN("Disconnected, attempting reconnect to" << _url.toString())

        emit tryReconnect(_url.toString());
