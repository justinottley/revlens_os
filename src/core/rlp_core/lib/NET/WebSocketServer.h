...
public slots:
    ...
    bool addClientToBroadcastGroup(
        QString clientIdent,
        QString broadcastGroupName,
        QVariantMap clientMetadata=QVariantMap()
    );
    ...
    QVariantList getBroadcastGroupClientInfo(QString broadcastGroupName);

    /*
     * Register some client metadata for the socket conenction.
     * Useful for adding to broadcasst message if client disconnects,
     * or for forensics / troubleshooting
     */
    void registerClientMetadata(QString clientIdent, QVariantMap clientMetadata);
...
private:
    ...
    QMap<QString, QSet<QString> > _broadcastGroupMap; // broadcast group -> all clients
    QMap<QString, QString> _clientBroadcastGroupMap; // client ident -> broadcast group
    QMap<QString, QVariantMap*> _clientMetadata;
