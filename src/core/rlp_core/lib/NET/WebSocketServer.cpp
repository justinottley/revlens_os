        ...
        _secure(secure)
{
    ...
    _clientBroadcastGroupMap.clear();
...
CoreNet_WebSocketServer::start()
{
    ...
    if (_wserver->listen(QHostAddress::AnyIPv4, _port)) { // 0 = Random port
        ...
        emit serverReady(_wserver->serverPort());
    }
}
...
void
CoreNet_WebSocketServer::registerClientMetadata(QString clientIdent, QVariantMap clientMetadata)
{
    RLP_LOG_DEBUG(clientIdent << clientMetadata)

    if (!_clientMetadata.contains(clientIdent))
    {
        QVariantMap* md = new QVariantMap();
        _clientMetadata.insert(clientIdent, md);
    }

    QVariantMap* cm = _clientMetadata.value(clientIdent);
    cm->insert(clientMetadata);
}
...
CoreNet_WebSocketServer::onNewConnection()
{
    ...
    QVariantMap* clientMetadata = new QVariantMap();
    _clientMetadata.insert(si, clientMetadata);
}
...
CoreNet_WebSocketServer::onSocketDisconnected()
{
    ...
    if (pClient)
    {
        ...
        if (_clientBroadcastGroupMap.contains(si))
        {
            QString broadcastGroup = _clientBroadcastGroupMap.value(si);
            removeClientFromBroadcastGroup(si, broadcastGroup);
        }
...
CoreNet_WebSocketServer::sendCallToClient(QVariantMap data)
{
    ...
    // RLP_LOG_DEBUG(clientIdent << msgStr)
...
CoreNet_WebSocketServer::sendBinaryToClient(QString clientIdent, const QByteArray& data)
{
    if (_clients.contains(clientIdent))
    {
        ...
        _clients.value(clientIdent)->flush();
    } else 
    {
        RLP_LOG_WARN("No client with ident" << clientIdent << " - skipping send")
    }
...
CoreNet_WebSocketServer::broadcast(QVariantMap data, QString broadcastGroup, QString skipIdent)
{
    ...
    } else
    {
        ...
        // RLP_LOG_DEBUG("BROADCAST GROUP: CLIENTS:" << clients << "skipident:" << skipIdent)
...
bool
CoreNet_WebSocketServer::addClientToBroadcastGroup(
    QString clientIdent,
    QString broadcastGroup,
    QVariantMap clientMetadata)
{
    ...
    _broadcastGroupMap.insert(broadcastGroup, s);
    _clientBroadcastGroupMap.insert(clientIdent, broadcastGroup);

    registerClientMetadata(clientIdent, clientMetadata);
...
CoreNet_WebSocketServer::removeClientFromBroadcastGroup(QString clientIdent, QString broadcastGroup)
{
    ...
    if (_clientBroadcastGroupMap.contains(clientIdent))
    {
        QString broadcastGroup = _clientBroadcastGroupMap.value(clientIdent);
        QVariantMap clientInfo;
        clientInfo.insert("broadcast.group_name", broadcastGroup);
        if (_clientMetadata.contains(clientIdent))
        {
            QVariantMap* md = _clientMetadata.value(clientIdent);
            clientInfo.insert(*md);
        }
        
        
        QVariantMap ddata;
        ddata.insert("method", "client_leave");
        ddata.insert("kwargs", clientInfo);
        
        QVariantMap msgEnvelope;
        msgEnvelope.insert("status", "OK");
        msgEnvelope.insert("result", ddata);

        // RLP_LOG_DEBUG("Broadcasting group leave:" << broadcastGroup << clientInfo)
        broadcast(msgEnvelope, broadcastGroup, clientIdent);

        _clientMetadata.remove(clientIdent);
        _clientBroadcastGroupMap.remove(clientIdent);
    }
...
QVariantList
CoreNet_WebSocketServer::getBroadcastGroupClientInfo(QString broadcastGroupName)
{
    // RLP_LOG_DEBUG(broadcastGroupName)

    QVariantList result;

    if (!_broadcastGroupMap.contains(broadcastGroupName))
    {
        RLP_LOG_ERROR("Not found:" << broadcastGroupName)
        return result;
    }

    QSet<QString> clients = _broadcastGroupMap.value(broadcastGroupName);

    // RLP_LOG_DEBUG("Got" << clients.size() << "clients")
