...
UTIL_Convert::toJsonObject(QVariantMap mapIn, QJsonObject* result)
{
    ...
    for (mit = mapIn.begin(); mit != mapIn.end(); ++mit) {
        ...
        if (typeName == "int") {
            ...
            //
            RLP_LOG_VERBOSE("Unknown type: " << typeName)
        }
    }
}
...
QString
UTIL_Convert::serializeToJson(QVariantList listIn)
{
    QJsonArray result;
    toJsonArray(listIn, &result);
...
QVariantMap
UTIL_Convert::jsonLoads(QString jsonStr)
{
    return UTIL_Convert::fromJsonString(jsonStr);
}


QString
UTIL_Convert::b64decode(QString b64EncodedString)
{
    RLP_LOG_DEBUG("")

    QString result = QByteArray::fromBase64(b64EncodedString.toLatin1());
