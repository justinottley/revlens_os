...
void
GUI_View::destroyScene()
{
    RLP_LOG_DEBUG("")

    _rootPane->deleteLater();
}


void
GUI_View::registerToolInfoMap(ToolInfoMap* tools)
{
    RLP_LOG_DEBUG("")
    if (_tools != nullptr)
    {
    ...
    ToolInfoMapIterator it;
    for (it = _tools->begin(); it != _tools->end(); ++it)
    {
        emit toolAdded(it.key());
    }
}
...
void
GUI_View::setViewScreen(QScreen* screen)
{
    RLP_LOG_DEBUG(screen->name() << screen->devicePixelRatio())

    setScreen(screen);

    emit screenChanged(screen->devicePixelRatio());
}
...
void
GUI_View::tabletEvent(QTabletEvent* event)
{
    // RLP_LOG_DEBUG(event)
    ...
    if (!event->isAccepted())
    {
        _scene->tabletEventInternal(event);
    }
}
