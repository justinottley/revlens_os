...
int
GUI_Pane::getNextVisibleTabIndex()
{
    for (int ti = 0; ti != _header->headerCount(); ++ti)
    {
        if (_header->tabHeader(ti)->isVisible())
        {
            return ti;
        }
    }
...
GUI_Pane::onTabCloseRequested(QString tabName, bool destruct)
{
    ...
    if (_tabs.size() > 0)
    {
        int nextVisibleTabIndex = getNextVisibleTabIndex();
        if (nextVisibleTabIndex > 0)
        {
            setActiveTab(nextVisibleTabIndex);
        } else
        {
            RLP_LOG_WARN("No more active visible tabs")
            _tabs.at(tabIndex)->setVisible(false);
...
GUI_Pane::mousePressEvent(QMouseEvent *event)
{
    ...
    if ((parentItem() != nullptr) && (parentItem()->metaObject()->className() == QString("GUI_PaneBody")))
    {
        ...
        if (_header->actButton()->menu() != nullptr)
        {
            ...
            _header->actButton()->menu()->setPos(epos.x(), epos.y());
