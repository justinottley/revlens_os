...
GUI_PaneHeader::initActButton()
{
    ...
    _panLeftButton = new GUI_SvgButton(":/feather/arrow-left.svg",
        this,
        20,
        3
    );
    ...
    _panRightButton = new GUI_SvgButton(":/feather/arrow-right.svg",
        this,
        20,
        3
    );
...
GUI_PaneHeader::onActionItemSelected(QVariantMap sigData)
{
    ...
    if (itemType == "tool")
    {
        ...
        if (item->inherits("GUI_ItemBase"))
        {
            ...
            QVariant v;
            v.setValue(this);
            iitem->setMetadata("pane.header", v);
...
GUI_PaneHeader::onPanLeft(QVariantMap md)
{
    ...
    GUI_TabHeader* lheader = _tabHeaders.at(0);
    float lhScenePosX = lheader->position().x();
    if (lhScenePosX < _panInc)
    {
        _xpan += _panInc;
        updateTabHeaders();
    }
...
GUI_PaneHeader::onPanRight(QVariantMap md)
{
    ...
    GUI_TabHeader* lheader = _tabHeaders.at(_tabHeaders.size() - 1);
    float lhScenePosX = lheader->mapToScene(lheader->position()).x();
    float phScenePosX = mapToScene(position()).x() + width();

    if (lhScenePosX > phScenePosX)
    {
        _xpan -= _panInc;
        updateTabHeaders();
    }
