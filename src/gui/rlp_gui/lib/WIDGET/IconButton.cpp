        ...
        _hoverCol(GUI_Style::BgLightGrey)
{
    ...
    {
        _svgIconRight = false;
        setSvgIconPath(filename, size);
        setWidth(_svgIcon->width());
    }
    else if (filename != "")
    {
...
void
GUI_IconButton::connectToMenu()
{
    connect(
        this, &GUI_IconButton::buttonPressed,
        this, &GUI_IconButton::toggleMenu
    );
}


void
GUI_IconButton::toggleMenu()
{
    ...
    qreal xpos = ppos.x() + _menuXOffset;
    qreal ypos = pos.y() + height() + _menuYOffset; 

    _menu->setPos(xpos, ypos);
...
void
GUI_IconButton::setIconPath(QString path)
{
    RLP_LOG_DEBUG(path)

    _icon = QImage(path).scaledToHeight(_iconSize, Qt::SmoothTransformation);
}
...
GUI_IconButton::setSvgIconPath(QString path, int size, bool changeColour)
{
    ...
    if (changeColour)
    {
        _svgIcon->setBgColour(GUI_Style::IconBg);
    }
    ...
    if (_svgIconRight)
    {
        _svgIcon->setPos(nwidth - _svgIcon->width() - _padding, 6);
    }
}


void
GUI_IconButton::setSvgIcon(GUI_SvgIcon* icon)
{
    _svgIcon = icon;
...
GUI_IconButton::onButtonPressed(QVariantMap metadata)
{
    ...
    if (menu->itemCount() > 0)
    {
        ...
        menu->setPos(
            ppos.x() + _menuXOffset,
            spos.y() + height() + _menuYOffset
        );

        scene()->hideMenuPanes(menu);
...
GUI_IconButton::hoverLeaveEvent(QHoverEvent* event)
{
    ...
    if ((_svgIconShowOnHover) && (_autoHideMenu))
    {
...
GUI_IconButton::hideMenu()
{
    if ((_menu->isVisible()) &&
        (!_inHover) &&
        (!qobject_cast<GUI_MenuPane*>(_menu)->isFocused()))
    {
