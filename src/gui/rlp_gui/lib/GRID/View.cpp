...
GUI_GRID_View::GUI_GRID_View(GUI_ItemBase* parent, GUI_GRID_ToolbarInit* tbi):
    ...
    _autoResize(true),
    _editable(false),
    _vscrollbarPolicy(GRID_VSCROLLBAR_POLICY_COLMODEL_WIDTH),
    _currStashedRowList(""),
    _selected(nullptr),
    _selectedRow(-1),
    _currFilterStr("")
...
void
GUI_GRID_View::setModelData(CoreDs_ModelDataItemPtr data, QString stashName, bool clearAllData, bool emitModelDataChanged)
{
    ...
    } else
    {
        ...
        {
            _rowList->at(i)->setVisible(false);
        }
        _rowList = new RowList();
    }
    ...
    for (int i=0; i != data->size(); ++i)
    {
        ...
        data->at(i)->insert("row_idx", i);

        QVariantMap rowInfo;
        GUI_GRID_RowPtr r(rf->makeRow(rowInfo, _scrollArea->content(), this, i));
    ...
    if (stashName != "")
    {
        stashCurrentRowList(stashName);
    }

    updateRows();

    if (emitModelDataChanged)
    {
...
GUI_GRID_View::setSelection(CoreDs_ModelDataItemPtr data, int rowIdx)
{
    ...
    updateSelection(data, rowIdx, GRID_SEL_REPLACE);
...
void
GUI_GRID_View::updateSelection(CoreDs_ModelDataItemPtr data, int rowIdx, GridSelectionMode selMode, bool emitSignal)
{
    // RLP_LOG_DEBUG(rowIdx << (int)selMode)
    ...
    if (selMode == GRID_SEL_APPEND_ALL)
    {
        startRow = _selectedRow;

        // e.g., bottom to top shift selection instead of top to bottom
        if (startRow > endRow)
        {
            startRow = rowIdx;
            endRow = _selectedRow;
    ...
    if (selMode == GRID_SEL_APPEND)
    {
        _fullSelection.append(result);
    ...
    if (emitSignal)
    {
        // RLP_LOG_DEBUG("sanity:" << _fullSelection.size() << _selectedRows.size())

        int fullSelSize = _fullSelection.size();
        int rowSelSize = _selectedRows.size();
        ...
        ModelItemIntMap selRowIdxMap;

        for (int i=0; i != fullSelSize; ++i)
        {
            CoreDs_ModelDataItemPtr selVal = _fullSelection.at(i);
            int rowIdx = selVal->value("row_idx").toInt();

            selRowIdxMap.insert(rowIdx, selVal);
            ...
            rsel.append(selVal->values());
        }

        QVariantList remResult;
        for (int ri = minRow; ri <= maxRow; ++ri)
        {
            if (!selRowIdxMap.contains(ri))
            {
                CoreDs_ModelDataItemPtr remVal = _data->at(ri);
                remResult.push_back(remVal->values());
            }
        }

        // emit "rem" selection event for entries that are inbetween
...
void
GUI_GRID_View::stashCurrentRowList(QString stashName)
{
    RLP_LOG_DEBUG(stashName)

    _rowListMap.insert(stashName, _rowList);
    _rowDataMap.insert(stashName, _data);
}


bool
GUI_GRID_View::hasStashedRowList(QString stashName)
{
    // RLP_LOG_DEBUG(stashName)

    return _rowListMap.contains(stashName);
}


void
GUI_GRID_View::clearStashedRowList(QString stashName)
{
    if (_rowListMap.contains(stashName))
    {
        RLP_LOG_DEBUG(stashName)

        RowList* rl = _rowListMap.value(stashName);

        if (rl == _rowList)
        {
            RLP_LOG_WARN("Resetting active row list")
            _rowList = new RowList();
        }
        
        for (int ri = 0; ri != rl->size(); ++ri)
        {
            rl->at(ri)->deleteLater();
        }

        _rowListMap.remove(stashName);

    } else
    {
        RLP_LOG_WARN("No stashed row list:" << stashName)
    }
}


GUI_GRID_Row*
GUI_GRID_View::rowAtStashedRowList(QString stashName, int rowIdx)
{

    // RLP_LOG_DEBUG(stashName << rowIdx)

    if (!_rowListMap.contains(stashName))
    {
        RLP_LOG_ERROR("No stashed row list:" << stashName)
        return nullptr;
    }

    if ((rowIdx < 0) || (_rowListMap.value(stashName)->size() <= rowIdx))
    {
        RLP_LOG_ERROR("Invalid row index for stashed row list" << stashName << ":" << rowIdx)
        return nullptr;
    }

    GUI_GRID_RowPtr r = _rowListMap.value(stashName)->at(rowIdx);
    if (r)
    {
        return r.get();
    }

    RLP_LOG_ERROR("Invalid row at" << rowIdx << "for stashed row list" << stashName)
    return nullptr;
}


bool
GUI_GRID_View::loadStashedRowList(QString stashName)
{
    if (!_rowListMap.contains(stashName))
    {
        RLP_LOG_ERROR("No stashed row list:" << stashName)
    ...
    if (!_rowDataMap.contains(stashName))
    {
        RLP_LOG_ERROR("No stashed row list:" << stashName)
        return false;
    }
    ...
    RowList* stashedRowList = _rowListMap.value(stashName);
    _rowList = stashedRowList;

    CoreDs_ModelDataItemPtr stashedRowData = _rowDataMap.value(stashName);
    _data = stashedRowData;

    for (int i=0; i != _rowList->size(); ++i)
    {
        _rowList->at(i)->setVisible(true);
    }

    _currStashedRowList = stashName;
...
void
GUI_GRID_View::setCurrStashedRowListName(QString stashName)
{
    RLP_LOG_DEBUG(stashName)
...
GUI_GRID_View::onSizeChangedInternal()
{
    ...
    int colModelWidth = _colModel->colWidth();

    _scrollArea->onParentSizeChanged(colModelWidth, height());

    if (_vscrollbarPolicy == GRID_VSCROLLBAR_POLICY_PARENT_WIDTH)
    {
        int scrollHPos = colModelWidth - 20;

        int mwidth = width();
        if (mwidth < colModelWidth)
        {
            scrollHPos = mwidth - 20;
        }

        GUI_VerticalScrollBarHandle* vh = _scrollArea->vhandle();
        QPointF vhPos = vh->position();
        vh->setPos(scrollHPos, vhPos.y());
    }
}


void
GUI_GRID_View::forceRefresh()
{
