...
GUI_ScrollBarHandle::GUI_ScrollBarHandle(GUI_ScrollArea* parent):
    GUI_ItemBase(parent),
    _bgCol(GUI_Style::BgBrightGrey),
    _hoverBgCol(GUI_Style::IconBg),
    _hover(false),
    _posPercent(0.0),
    _opacity(1.0),
    _autoScroll(true)
...
GUI_ScrollBarHandle::paint(GUI_Painter* painter)
{
    ...
    if (_hover)
    {
        painter->setPen(_hoverBgCol);
        painter->setBrush(_hoverBgCol);
...
/*
void
GUI_ScrollAreaContent::setBgColour(QColor col)
{
    RLP_LOG_DEBUG(col)
...
GUI_ScrollArea::GUI_ScrollArea(QQuickItem* parent, qreal width, qreal height):
    ...
    _maxChildHeight(0),
    _maxChildWidth(0),
    _contentWidthOffset(0),
    _autoHide(true),
    _inHover(false),
    _outlined(true),
    _hScrollBarVisible(true),
    _hScrollbarVisOffset(30),
    _vhandlePosOffset(0),
    _bgCol(Qt::black)
{
...
GUI_ScrollArea::onParentSizeChanged(qreal width, qreal height)
{
    ...
    _content->setWidth(width - _contentWidthOffset);

    _contentWrapper->setWidth(width - _contentWidthOffset);
...
GUI_ScrollArea::setMaxChildHeight(qreal nheight)
{
    ...
    _content->setHeight(_maxChildHeight);
...
{
    // RLP_LOG_DEBUG(_maxChildWidth << width() << _hScrollBarVisible)

    if ((_autoHide) && (_maxChildWidth < (width() - _hScrollbarVisOffset)))
    {
...
GUI_ScrollArea::setHScrollBarVisible(bool isVisible)
{
    ...
    _hhandle->setVisible(_hScrollBarVisible);
}
...
{
    // RLP_LOG_DEBUG(event);
    event->setAccepted(false); // propagate
}
