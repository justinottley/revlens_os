...
RLP_SETUP_LOGGER(gui, GUI, SvgUtil)
...
QImage ChangeSVGColor(QString iconPath, int size, QString color, bool changeColour)
{
    ...
    // TODO FIXME: NanoPainter filename hash doesn't work for same image with different colors
    // add color to hash??
    //
...
GUI_SvgUtil::GUI_SvgUtil()
{
}


QImage
GUI_SvgUtil::renderSvg(QString svgPath, int size)
{
    return ChangeSVGColor(svgPath, size, "", false);
}
...
GUI_SvgIcon::setBgColour(QColor col)
{
    ...
    _bgCol = col;
    _bgImage = ChangeSVGColor(_filename, _size, toSvgCol(_bgCol), true);
}
...
GUI_SvgIcon::setFgColour(QColor col)
{
    ...
    _fgCol = col;
    _fgImage = ChangeSVGColor(_filename, _size, toSvgCol(_fgCol), true);
}
...
GUI_SvgIcon::mousePressEvent(QMouseEvent* event)
{
    ...
    if (_doForwardMouseEvent)
    {
        emit forwardMouseEvent(event);
    }
    else
    {
